...
window.Cartographer = {
    ...
    getRoadTypeDef: function(type) {
        const roadTypes = window.gamedata?.map_markers?.road_types;
        if (!Array.isArray(roadTypes)) return null;
        return roadTypes.find(roadType => roadType.id === type) || null;
    },

    getRoadTypeStyle: function(road) {
        const isRuined = road.condition === 'ruined' || road.integrity < 30;
        const roadDef = this.getRoadTypeDef(road.type);

        if (roadDef) {
            return {
                lineWidth: roadDef.line_width || roadDef.width || 2,
                strokeStyle: isRuined ? (roadDef.ruined_color || '#7e3f37') : (roadDef.color || '#6f5b40'),
                lineDash: isRuined ? (roadDef.ruined_dash || roadDef.dash || []) : (roadDef.dash || [])
            };
        }

        if (road.type === 'bridge') {
            return { lineWidth: 4, strokeStyle: isRuined ? '#7e3f37' : '#6a5846', lineDash: isRuined ? [4, 4] : [] };
        }
        if (road.type === 'tunnel') {
            return { lineWidth: 3, strokeStyle: isRuined ? '#7e3f37' : '#4a4640', lineDash: [6, 4] };
        }
        if (road.type === 'ferry') {
            return { lineWidth: 2, strokeStyle: isRuined ? '#7e3f37' : '#345468', lineDash: [4, 4] };
        }
        if (road.type === 'highway') {
            return { lineWidth: 3, strokeStyle: isRuined ? '#7e3f37' : '#7f7665', lineDash: isRuined ? [5, 5] : [] };
        }
        if (road.type === 'sea_route') {
            return { lineWidth: 2, strokeStyle: '#345468', lineDash: [6, 6] };
        }
        if (road.type === 'paved' || road.condition === 'paved') {
            return { lineWidth: 2, strokeStyle: isRuined ? '#7e3f37' : '#7f7665', lineDash: isRuined ? [5, 5] : [] };
        }

        return {
            lineWidth: 2,
            strokeStyle: isRuined ? '#7e3f37' : '#6f604e',
            lineDash: isRuined ? [3, 6] : [8, 4]
        };
    },

    getBridgeFlagOverlaySegments: function(map) {
        const segments = [];
        if (!map || !Array.isArray(map.grid)) return segments;

        for (let y = 0; y < map.height; y++) {
            for (let x = 0; x < map.width; x++) {
                const cell = map.grid[y * map.width + x];
                const bridgeFlag = Array.isArray(cell) ? cell[2] : (cell && cell.bridge_flag);
                if (bridgeFlag) {
                    segments.push({
    ...
    updateBackgroundCache: function(map) {
        ...
        if (map.roads) {
            map.roads.forEach(road => {
                ...
                const roadStyle = this.getRoadTypeStyle(road);
                ctx.lineWidth = roadStyle.lineWidth;
                ctx.strokeStyle = roadStyle.strokeStyle;
        ...
        const bridgeFlagSegments = this.getBridgeFlagOverlaySegments(map);
        bridgeFlagSegments.forEach(segment => {
            ctx.lineWidth = 2;
            ctx.strokeStyle = '#b28a58';
            ctx.setLineDash([]);
            ctx.beginPath();
            ctx.moveTo(segment.x - 4, segment.y - 2);
            ctx.lineTo(segment.x + 4, segment.y - 2);
            ctx.stroke();
            ctx.beginPath();
            ctx.moveTo(segment.x - 4, segment.y + 2);
            ctx.lineTo(segment.x + 4, segment.y + 2);
            ...
            ctx.beginPath();
            ctx.moveTo(segment.x - 3, segment.y - 4);
            ctx.lineTo(segment.x + 3, segment.y + 4);
            ctx.stroke();
        });
