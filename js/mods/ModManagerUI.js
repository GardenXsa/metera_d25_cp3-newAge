...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('contextmenu', (e) => {
        // Only destroy if right-clicking outside a mod card
        if (!e.target.closest('.mm-card')) {
            destroyContextMenu();
        }
    });

    function showModContextMenu(e, mod, isActive, index) {
        e.preventDefault();
        e.stopPropagation();
        destroyContextMenu();
        ...
        if (isActive) {
            items.push({ icon: 'fa-chevron-up', label: 'Поднять приоритет', action: () => { if (index > 1) moveModUp(index); }, disabled: index <= 1 });
            items.push({ icon: 'fa-chevron-down', label: 'Опустить приоритет', action: () => moveModDown(index), disabled: index >= activeModIds.length - 1 });
            items.push({ type: 'separator' });
            items.push({ icon: 'fa-minus', label: 'Отключить мод', action: () => toggleMod(mod.id, false), danger: true });
        } else {
            if (mod.runtimeDisabled) {
                items.push({ icon: 'fa-undo', label: 'Сбросить автоотключение и включить', action: () => toggleMod(mod.id, true) });
            } else {
                items.push({ icon: 'fa-plus', label: 'Включить мод', action: () => toggleMod(mod.id, true) });
            }
        }

        // Common items
        items.push({ type: 'separator' });
        items.push({ icon: 'fa-folder-open', label: 'Открыть папку мода', action: () => {
            if (window.electronAPI && window.electronAPI.modsOpenFolder) {
                window.electronAPI.modsOpenFolder();
            }
        }});

        if (mod.id !== 'base_game') {
            items.push({ icon: 'fa-trash', label: 'Удалить мод', action: () => {
                if (confirm(`Удалить мод "${mod.name || mod.id}"? Это действие необратимо!`)) {
                    // Disable first, then user can manually delete folder
                    if (isActive) toggleMod(mod.id, false);
                    if (window.RuntimeLog) {
                        window.RuntimeLog.info('ModManagerUI', `Пользователь запросил удаление мода ${mod.id}`);
                    }
                    window.electronAPI.modsOpenFolder();
                }
            }, danger: true });
        ...
        items.forEach(item => {
            if (item.type === 'separator') {
                ...
                sep.className = 'mm-ctx-separator';
                menu.appendChild(sep);
                return;
            }
            const btn = document.createElement('div');
            btn.className = 'mm-ctx-item' + (item.danger ? ' danger' : '');
    ...
    function createModElement(mod, isActive, index, errorMsg, _pendingChanges) {
        ...
        el.addEventListener('contextmenu', (e) => showModContextMenu(e, mod, isActive, index));
        return el;
    }
