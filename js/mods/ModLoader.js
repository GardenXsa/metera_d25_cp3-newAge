                ...
                window.ModAPI.on('onDatabaseLoad', async (db) => {
                    ...
                    for (const [rawKey, fileList] of Object.entries(mod.data)) {
                        ...
                        if (mod.total_conversion && descriptor && descriptor.replace_on_total_conversion === true && !mod._runtimeResetKeys?.includes(targetKey)) {
                            const tcContract = runtimeManifest && runtimeManifest.modding_contract ? runtimeManifest.modding_contract : {};
                            ...
                            if (passthroughKeys.includes(targetKey)) {
                                if (window.RuntimeLog) {
                                    window.RuntimeLog.info('ModLoader', `[ModLoader] Skip TC reset for passthrough key '${targetKey}' (kept base data from ${descriptor.path || 'base_game'})`, { modId, targetKey });
                                } else {
                                    console.log(`[ModLoader] Skip TC reset for passthrough key '${targetKey}' (kept base data from ${descriptor.path || 'base_game'})`);
                                }
                            } else {
