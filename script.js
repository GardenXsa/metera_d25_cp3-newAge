    ...
    _systemContainerRegistryPromise = (async () => {
        ...
        if (typeof fetch === 'function') {
            try {
                ...
                console.warn('[DataArch]    data/system_containers.json:', error.message);
            }
        }
...
async function ensurePlayerContainers() {
    ...
    if (needsBackpack) {
        console.warn("[Inventory]       .   data/system_containers.json.");
        player.container_backpack = await createConfiguredSystemContainer('player_backpack');

        if (!player.container_backpack || !ContainerRegistry.has(player.container_backpack)) {
            throw new Error("[Inventory] failed: player_backpack  data/system_containers.json");
        }
    }
    ...
    if (needsEquipment) {
        ...
        if (!player.container_equipment || !ContainerRegistry.has(player.container_equipment)) {
            throw new Error("[Inventory] failed: player_equipment  data/system_containers.json");
        }
    }
...
async function ensureGuardConfiscationChest() {
    ...
    if (!createdId || !ContainerRegistry.has(createdId)) {
        throw new Error("[Inventory] failed: guard_confiscation_chest  data/system_containers.json");
    }
    ...
    unlockContainer: function(containerId, actorId) {
        ...
        if (roll >= cont.lock_data.difficulty) {
            ...
            if (cont.lock_data.trap) {
                ...
                trapMsg = ` Trap triggered! Damage: ${cont.lock_data.trap.amount}.`;
            }
...
async function sendInventoryCommand(action, args, _retryCount = 0) {
    ...
    if (!window.electronAPI || !window.electronAPI.nexusInventoryCommand) {
        // FALLBACK: IPC      (OldCoreInventorySystem)
...
/**
 *     fallback  C++  / IPC .
 *   OldCoreInventorySystem (   ContainerRegistry / ItemRegistry).
 */
...
async function equipItemAsync(itemId, targetSlot = null) {
    ...
    if (!bodySlots.includes(targetSlot)) return `[ERROR]     : '${targetSlot}'`;
...
const TransportSystem = {
    ...
    async init() {
        if (!this.registry) {
            try {
                ...
                // Fallback    
                this.registry = {
                    horse: { id: 'horse', speedMultiplier: 2.0, cargoBonus: 5, name: 'Horse', basePrice: 500, rarity: '' },
                    warhorse: { id: 'warhorse', speedMultiplier: 1.8, cargoBonus: 3, name: 'Warhorse', basePrice: 1200, rarity: '' },
                    cart: { id: 'cart', speedMultiplier: 1.3, cargoBonus: 15, name: 'Cart', basePrice: 300, rarity: '' },
                    wagon: { id: 'wagon', speedMultiplier: 1.5, cargoBonus: 30, name: ' ', basePrice: 500, rarity: '' },
                    ship_deed: { id: 'ship_deed', speedMultiplier: 2.5, cargoBonus: 50, name: 'Ship Deed', basePrice: 2000, rarity: '' }
                };
            }
        }
    ...
    generateGMDocumentation() {
        ...
        let doc = '=== [    -   ] ===\n';
        doc += ':  ID (!)\n';
        doc += '    addItem    aiIdentifier:\n\n';

        for (const [id, data] of Object.entries(this.registry)) {
            doc += `  "${id}" - ${data.name || data.nameEn} ( ${data.speedMultiplier}, +${data.cargoBonus} )\n`;
        }

        doc += '\n:\n';
        doc += '  { "command": "addItem", "args": { "aiIdentifier": "horse", "name": " " } }\n';
        doc += '  { "command": "mountTransport", "args": { "itemId": "horse" } }\n';
        doc += '\n:\n';
        doc += '  { "command": "addItem", "args": { "aiIdentifier": "horse_brown" } } \n';
        doc += '  { "command": "addItem", "args": { "aiIdentifier": "" } } \n';
        doc += '=== [  ] ===\n';
...
const TradeSystemAsync = {
    ...
    _toggleConfirmButton: function(tradeId = null) {
        ...
        btn.onclick = async () => {
            ...
            addLogMessage(res.success ? "[]      ." : `[ERROR]  : ${res.error}`, "system-message");
...
async function executeCommand(command, args) {
    ...
    console.log("  (ASYNC):", command, args);
    let feedback = null;
    ...
    try {
        ...
        if (window.ModAPI) ModAPI.emit('onQuestUpdated', {action: 'add', quest: args});
            ...
            case 'addItem':
                if (args.aiIdentifier && args.name) {
                    ...
                    if (cont.owner_id !== 'player' && currentWeight + addedWeight > cont.max_weight_kg) {
                        feedback = `[ ]  ! : ${cont.max_weight_kg} .   ${quantity} . '${name}' (: ${addedWeight.toFixed(2)} ).  ,   !`;
                        break;
                    }
                    ...
                    if (getContainerItems(cont).length >= cont.max_slots) {
                            ...
                            const customProps = {
                                ...
                                rarity: args.rarity || (transportData?.rarity) || '',
                            ...
                            if (!isTransport && (aiId.toLowerCase().includes('horse') || aiId.toLowerCase().includes('cart') || aiId.toLowerCase().includes('wagon'))) {
                                ...
                                feedback = t('gameInterface.commandFeedback.itemAdded', { itemName: name, quantity: quantity }) + ` [WARNING] ID "${aiId}"   . : ${validIds.join(', ')}`;
                            } else {
                                await CoreInventorySystemAsync.createItem(aiId, quantity, targetContId, customProps);
                    ...
                    feedback = `[ERROR] 'addItem'  'aiIdentifier'  'name'.`;
                }
                break;
                ...
                if (!rawId) {
                    feedback = `[ERROR] 'removeItem'  'itemId'  'aiIdentifier'.`;
                    break;
                }
                ...
                if (!rawId) {
                    feedback = `[ERROR] 'equipItem''aiIdentifier'.`;
                    break;
                }
            ...
            case 'unequipItem':
                if (args.slot) {
                    ...
                    feedback = `[ERROR] 'unequipItem'  'slot'.`;
                }
                break;
            ...
            case 'createContainer':
                if (args.type && args.ownerId) {
                    ...
                    feedback = `[]   ${contId}  ${args.type}  ${args.ownerId}.`;
                } else {
                    feedback = `[ERROR] 'createContainer'  'type'  'ownerId'.`;
                }
                break;
            ...
            case 'moveItem':
                if (args.itemId && args.sourceContainerId) {
                    ...
                    feedback = res.success ? `[]  .` : `[ERROR]  : ${res.error}`;
                    updateInventoryDisplay();
                    updateEquipmentDisplay();
                    updateCharacterSheet();
                } else {
                    feedback = `[ERROR] 'moveItem'  'itemId'  'sourceContainerId'.`;
                }
                break;
            ...
            case 'move_items':
                if (args.sourceContainerId && Array.isArray(args.items) && args.items.length > 0) {
                    ...
                    feedback = `[ERROR] 'moveItems'  'sourceContainerId'  'items' [{id, quantity}].`;
                }
                break;
            ...
            case 'initiate_trade': {
                ...
                feedback = res.success ? `[] ${res.message}` : `[ERROR] ${res.error}`;
                break;
            }
            ...
            case 'negotiate': {
                ...
                feedback = res.success
                    ? `[]   ${res.acceptedPrice ? `: ${res.acceptedPrice} .` : '.'}`
                    : `[ERROR]   : ${res.error}`;
                break;
            }
            ...
            case 'destroyContainer':
                if (args.containerId) {
                    ...
                    feedback = res ? `[]  ${args.containerId} ,    .` : `[ERROR]   .`;
                    updateInventoryDisplay();
                    updateEquipmentDisplay();
                    updateCharacterSheet();
                } else {
                    feedback = `[ERROR] 'destroyContainer'  'containerId'.`;
                }
                break;
            ...
            case 'unlockContainer':
                if (args.containerId) {
                    ...
                    feedback = res.success ? `[] : ${res.message}` : `[] : ${res.error}`;
                    updateInventoryDisplay();
                    updateCharacterSheet();
                } else {
                    feedback = `[ERROR] 'unlockContainer'  'containerId'.`;
                }
                break;
            ...
            case 'confiscateStolen':
                if (args.targetId) {
                    ...
                    feedback = `[ERROR] 'confiscateStolen'  'targetId'.`;
                }
                break;
            ...
            case 'buildContainer':
                if (args.type) {
                    ...
                    feedback = contId ? `[]   ${contId}.  5 .` : `[ERROR]   ( 5 wood).`;
                    updateInventoryDisplay();
                    updateCharacterSheet();
                } else {
                    feedback = `[ERROR] 'buildContainer'  'type'.`;
                }
                break;
            ...
            case 'applyAoEDamage':
                if (args.location && args.damage) {
                    ...
                    feedback = `[] AoE  (${args.damage})    ${args.location}.  : ${destroyed}.   .`;
                    updateInventoryDisplay();
                    updateEquipmentDisplay();
                    updateCharacterSheet();
                } else {
                    feedback = `[ERROR] 'applyAoEDamage'  'location'  'damage'.`;
                }
                break;
            ...
            case 'castMagicalPocket': {
                ...
                if (!player.statusEffects['spell_magical_pocket']) {
                    player.statusEffects['spell_magical_pocket'] = { id: 'spell_magical_pocket', name: ' ', duration: 9999, description: '    .', effects: [] };
                }
                if (existingPocket) {
            ...
            case 'fleePackAnimal':
                if (args.containerId) {
                    ...
                    feedback = `[ERROR] 'fleePackAnimal'  'containerId'.`;
                }
                break;
                ...
                if (rawId && args.stat && args.change !== undefined) {
                    ...
                    if (item) {
                        ...
                        feedback = `[]  '${args.stat}'  '${item.custom_props.name}'   ${change > 0 ? '+' + change : change}.`;
                        if (isEquipped) updateEquipmentDisplay();
                        else updateInventoryDisplay();
                    } else {
                        feedback = `[ERROR]  '${searchTerm}'    updateItemStat.`;
                    }
                } else {
                    feedback = `[ERROR] 'updateItemStat'  'aiIdentifier', 'stat'  'change'.`;
                }
                break;
            }
            ...
            case 'mountTransport': {
                ...
                if (!itemId) {
                    feedback = `[ERROR] 'mountTransport'  'itemId'.`;
                    break;
                }
        ...
        console.error(`     ${command} (ASYNC):`, error, args);
    }
    return feedback;
}
...
const OldTradeSystem = {
    ...
    _postProcessTrade: async function(trade) {
        ...
        if (World?.news) {
            World.news.push({
                category: 'trade',
                text: `     ${trade.target}   ${trade.final_price || 0} .`,
...
function getCaravanContents(chestId) {
    if (!chestId || !ContainerRegistry.has(chestId)) return "";
    const cont = ContainerRegistry.get(chestId);
    if (!cont.items || cont.items.length === 0) return "";
    let contents = [];
...
function formatTrekObjectData(objType, data) {
    ...
    if (objType === 'caravan') {
        ...
        return ` (ID: ${data.id}). : ${data.origin} -> ${data.destination}. : ${data.guards} .  (chest_id: ${data.chest_id}): ${goods}.`;
    } else if (objType === 'army') {
        return ` (ID: ${data.id}). : ${data.faction_name || ''}. : ${data.size}. : ${data.morale}.  : ${data.destination}. : ${data.current_phase}.  : ${data.supply_chest_id || ''}.`;
    }
...
const LivingRoads = {
    ...
    start: async function(destinationId) {
        if (!player) return { success: false, error: "  " };
        ...
        if (!dest) {
            ...
            addLogMessage(`[]  '${destinationId}'    .   ,    .`, "system-message");
        ...
        if (window.electronAPI && window.electronAPI.nexusStartTrek) {
            ...
            if (res.status === 'ok') {
                ...
                updateCharacterSheet();
                addLogMessage(`[]   ${dest.name} .  : ${res.total_hours} .`, "system-message");
            }
        }
        this.resume();
        return { success: true };
    ...
    pause: async function(reason = "manual") {
        ...
        updateCharacterSheet();
        addLogMessage(`[]  . : ${reason}`, "system-message");
    },
    ...
    fastForward: async function() {
        ...
        if (otherEvents.length > 0) {
            ...
            otherEvents.forEach(ev => {
                ...
                addLogMessage(`<div style="border-left: 3px solid #f39c12; padding-left: 10px; margin: 5px 0;"><strong style="color:#f39c12;">[  ]</strong> ${description}</div>`, "system-message");
            });
        }
    ...
    finish: function() {
        ...
        addLogMessage(`[]  .   : ${player.travel.destinationName}.`, "system-message");
        player.travel.active = false;
        updateCharacterSheet();
        updateMapDisplay();
        const prompt = `[SYSTEM:  ]     ${player.travel.destinationName}.     .`;
        sendApiRequest(prompt, false, false, [], false);
    }
};
...
async function preSimulateWorldHistory(yearsToSimulate) {
    ...
    if (window.electronAPI && window.electronAPI.nexusPreSimulate) {
        ...
        if (loadingText) loadingText.textContent = `   ${yearsToSimulate}  (  Nexus Engine)...`;

        console.log(`[Nexus]  - ${totalTicks}   C++...`);
        const res = await window.electronAPI.nexusPreSimulate(World, totalTicks);
        console.log(`[Nexus]  - :`, res ? `status=${res.status}` : 'null');
        if (res.status === 'ok') {
            if (res.world) setWorld(res.world);
            ...
            if (m.region_id === playerRegionId) {
                ...
                if (!player.quests[questId]) {
                    player.quests[questId] = {
                        ...
                        title: " : " + m.name,
                        objective: "" + (World.regions[m.region_id] ? World.regions[m.region_id].name : m.region_id),
                        description: "   .     .      .",
                        reward: " , ",
                        issuer: " ",
                        status: 'active'
                    ...
                    if (!IS_PRE_SIMULATING && !window.isSimulatingTime) {
                        addLogMessage(`[-]   :    ${m.name}!`, "system-message");
                        questsUpdated = true;
                    }
                }
            }
    ...
    Object.keys(player.quests).forEach(qId => {
        if (qId.startsWith("hunt_") && player.quests[qId].status === 'active') {
            ...
            if (!monster || monster.health <= 0) {
                ...
                if (!IS_PRE_SIMULATING && !window.isSimulatingTime) {
                    addLogMessage(`[-]  : ${player.quests[qId].title}!`, "level-up");
                    questsUpdated = true;
                }
            }
        }
        ...
        if (ticks > 0) {
            ...
            const startRealtimeIfNeeded = async () => {
                ...
                window.electronAPI.nexusSimulate(World, ticks, player?.location || "").then(res => {
                    ...
                    console.error("[Nexus]   nexusSimulate:", err);
                    if (window.isSimulatingTime) {
                        hideLoadingScreen();
                        window.isSimulatingTime = false;
                    }
...
async function runWorldSimulationTick() {
    ...
    loaderDiv.innerHTML = `
        <div class="astrolabe" style="filter: hue-rotate(120deg) brightness(0.8);">
            <div class="astrolabe-ring"></div><div class="astrolabe-ring"></div><div class="astrolabe-ring"></div><div class="astrolabe-core"></div>
                ...
                <span class="ether-text-title" style="color: #e74c3c; text-shadow: 0 0 10px #e74c3c;"> ...</span>
                <span class="ether-text-subtitle">   </span>
            </div>
    ...
    try {
        ...
        for (let rId in World.regions) {
            ...
            let ownerName = World.factions[r.factionId] ? World.factions[r.factionId].name : " ";
            ...
            let resStr = resArr.length > 0 ? resArr.slice(0, 6).join(', ') : "";

            worldSummary += `: ${r.name} (: ${ownerName}). : ${r.population}. : ${r.weather || ""}. : ${resStr}.\n`;
        }
        ...
        for (let fId in World.factions) {
            ...
            worldSummary += `: ${f.name}.   : ${manpower}.   : ${gold}.    : ${f.armies.length}.\n`;
            for (let target in f.diplomacy) {
                if (f.diplomacy[target] === "war") activeWars.push(`${f.name}   ${World.factions[target].name}`);
            }
        }
        if (activeWars.length > 0) worldSummary += `\n: ${[...new Set(activeWars)].join(", ")}\n`;
        ...
        let recentNews = World.news
            ...
            .map(n => `[${n.daysOld} . , : ${n.location}]: ${parseLocString(n.text)}`)
            .join("\n");
        worldSummary += `\n     :\n${recentNews || "  "}\n`;
        ...
        let currentDateStr = `${player.gameTime.day} ${mName}, ${player.gameTime.year} `;
        
        const prompt = `### :   (WORLD SIMULATOR) v5.0\n   .  :    ("  ")    .\n\n[ ]:\n-  : ${currentDateStr}\n-     :  ${daysPassed} .\n\n${worldSummary}\n\n (  ):\n1.   "  ".     "[X . ]".    14     4 ,    .   ,   "  4 "!\n2.   "  ".    ""   0 ,            .       ,     .\n3.     ,  ,    ${daysPassed} .     ,    ,     .\n4.        (: "  ${daysPassed} ...", " ${currentDateStr} ...").\n5.       JSON .  actions   [].\n:\n{\n  "narrative": "     ...",\n  "actions": []\n}`;
        ...
        const raw = await performAiFetch(prompt, [], modelId, `   ${daysPassed} .`);
...
const SOURCE_TILE_SIZE = 16; //     Kenney 1-bit
const SPACING = 1;           //     -
const RENDER_TILE_SIZE = 48; //    
...
async function loadTileSet() {
    try {
        ...
        if (response.ok) {
            ...
            if (data.mappings) {
                ...
                console.log(`[TileSet]   : ${Object.keys(TILE_SPRITE_MAP).length} .`);
            }
        } else {
            console.warn('[TileSet]    tileset.json');
        }
    } catch (e) {
        console.error('[TileSet]   tileset.json:', e);
    }
    ...
    return new Promise((resolve) => {
        ...
        img.onload = () => {
            TILESET_IMAGE = img;
            console.log('[TileSet]  - Kenney 1-Bit');
            resolve(true);
        };
        img.onerror = (err) => {
            console.error('[TileSet]   -,  fallback CSS', err);
...
//      
let bodySlots = []; //    window.EQUIPMENT_SLOTS
...
let worldLore = " ...";
let globalLocations = {};
let skillsReferenceData = "  ...";
let environmentCommandsGuideData = "    ...";
...
let promptVariablesCache = {};
    ...
    if (window.ModAPI && window.ModAPI.isTotalConversion) {
        console.log(`[Total Conversion]      ${eraId}.`);
...
function updateEchoMemoryDisplay() {
    ...
    if (!player || !player.echoMemory || !player.echoMemory.items || player.echoMemory.items.length === 0) {
        listEl.innerHTML = `<li data-i18n="gameInterface.echoMemoryPanel.empty">${t('gameInterface.echoMemoryPanel.empty', '  ')}</li>`;
        return;
    }
    ...
    player.echoMemory.items.forEach((item, idx) => {
        ...
        li.innerHTML = `
            ...
            <button class="echo-delete-btn" data-index="${idx}" title=""></button>
        `;
    ...
    if (clearBtn) {
        ...
        clearBtn.onclick = () => {
            showCustomConfirm(t('gameInterface.echoMemoryPanel.confirmClear', '  -?'), () => {
...
// ---     --- 
    ...
    if (DEBUG_MODE && player) {
        ...
        gmNotesContent.innerHTML = sanitizeHTML(displayHtml) || t('gameInterface.gmNotesPanel.empty', '  .');
    } else {
...
function populateRacesUI(racesData) {
    ...
    charRaceSelect.innerHTML = `<option value="" disabled ${!currentValue ? 'selected' : ''} data-i18n="characterCreation.racePlaceholder">${typeof t === 'function' ? t('characterCreation.racePlaceholder') : '--   --'}</option>`;
...
function populateClassesUI(classesData) {
    ...
    charClassSelect.innerHTML = `<option value="" disabled ${!currentValue ? 'selected' : ''} data-i18n="characterCreation.classPlaceholder">${typeof t === 'function' ? t('characterCreation.classPlaceholder') : '--   --'}</option>`;
...
// 1.   ( )
const quickTags = [
    { label: ' Attack', type: 'combat', stat: 'atk' },
    { label: ' Defend', type: 'combat', stat: 'def' },
    { label: ' D20', type: 'stat', stat: 'd20' },
    { label: ' STR', type: 'stat', stat: 'str' },
    { label: ' DEX', type: 'stat', stat: 'dex' },
    { label: ' INT', type: 'stat', stat: 'int' },
    { label: ' CON', type: 'stat', stat: 'con' },
    { label: ' CHA', type: 'stat', stat: 'cha' }
];
...
function createRollBadge(statKey, labelText) {
    ...
    badge.innerHTML = `
        ...
        <span class="roll-badge-close" title=" "></span>
    `;
...
function buildFullPlayerSnapshot() {
    ...
    const inHands = player.equipment.right_hand ? player.equipment.right_hand.name : '';
    ...
    if (typeof World !== 'undefined' && World && World.monsters && World.monsters.length > 0) {
        ...
        World.monsters.forEach(m => {
            if (m.health > 0) {
                worldContextString += ` ${m.name} (: ${m.type}, : ${m.level}, HP: ${m.health}/${m.maxHealth}, : ${m.attack}, : ${m.defense}). : ${m.region_id}. :  ${m.treasure_chest_id}.\n`;
            }
        });
        worldContextString += " :       ,   `addEnvironment`   .       `killMonster``monsterId`,      .\n==================================================\n";
    }
...
function updateReputationModal() {
    ...
    if (modalTitle) modalTitle.textContent = t('gameInterface.reputationModal.title', null, '');
...
function pruneGameLog() {
    const MAX_LOG_MESSAGES = 100; //   DOM   100 
                ...
                if (removedEffect.originalValues && typeof removedEffect.originalValues === 'object') {
                    for (const statToRestore in removedEffect.originalValues) {
                        ...
                        if (message) {
                            logMessages.push(message);
                            specificActionOccurred = true;
                            console.warn(`   '${statToRestore}''${removedEffect.name}', .. GM    on_remove.`);
                        }
                    }
                }
...
function checkEffectTrigger(effect, trigger) {
    ...
    //      : on_damage_taken, on_attack,  ..
    return false;
}

/**
 *      .
 * @param {object} entity - ,     (  player).
 * @param {object} effect -  - (  originalValues).
 * @param {object} action -  .
        ...
        switch (action.type) {
            ...
            case 'restore_stat': {
                ...
                if (effect.originalValues && effect.originalValues[stat] !== undefined) {
                    ...
                    delete effect.originalValues[stat]; //   
                }
                break;
            }
        }
 ...
 *      .
 * @param {string} slotType - 'manual'  'auto'.
 * @param {number} slotId - ID .
 */
...
function setActiveScreen(screenId) {
    ...
    //     
        ...
        if (currentActive) currentActive.classList.remove('active-screen'); //  - 
        setTimeout(() => {
            executeSwap();
...
function showAiErrorModal(errorText, isInitial, onRetry, customTitle = null, customDesc = null) {
    ...
    if (customTitle && titleH3) {
        ...
        titleH3.innerHTML = `<i class="fas fa-exclamation-triangle"></i>   `;
    }
    ...
    if (customDesc) {
        ...
        aiErrorCancelBtn.textContent = "";
    } else if (mainText) {
        // FIX:        
        aiErrorMessage.textContent = mainText;
        aiErrorCancelBtn.textContent = isInitial ? "  " : " (  )";
    } else if (isInitial) {
    ...
    aiErrorDetailsToggle.textContent = "  ";
    ...
    if (isRateLimitOrUnavailable && !isInitial) {
        aiErrorRetryBtn.textContent = "  (Fallback)  ";
...
function updateApiKeyStatus() {
    ...
    if (newGameButton) {
        ...
        newGameButton.title = keyIsMissing ? t('mainMenu.tooltips.newGameDisabled', ' API   ,  ') : t('mainMenu.tooltips.newGame', '  ');
    }
}
...
function saveApiKey() {
    ...
    if (newKey) {
        ...
        //  ALERT
        showCustomAlert(t('settingsMenu.apiKeySaved', null, 'API  !'));
    } else {
        localStorage.removeItem('geminiApiKey');
        GEMINI_API_KEY = '';
        //  ALERT
        showCustomAlert(t('settingsMenu.apiKeyRemovedOrEmpty', null, 'API  .'));
    }
    updateApiKeyStatus();
}
...
function playMusic(index) {
    ...
    if (playPromise !== undefined) {
        playPromise.then(_ => {
            ...
            console.warn(`     ${musicFiles[index]}:`, error);
...
function updateMusicToggleButton(isPlaying) {
    ...
    if (isPlaying) {
        ...
        toggleMusicButton.title = t('gameInterface.toggleMusicButtonTitlePause', "");
    } else {
        toggleMusicIcon.classList.remove('fa-volume-high', 'fa-pause');
        toggleMusicIcon.classList.add('fa-volume-off');
        toggleMusicButton.title = t('gameInterface.toggleMusicButtonTitlePlay', " ");
    }
...
function loadTTSVoices() {
    //    ,      assets/tts
    ttsVoices = [
        { name: " (, )", file: "ru_RU-irina-medium.onnx", lang: "ru" },
        { name: " (, )", file: "ru_RU-dmitri-medium.onnx", lang: "ru" },
...
function handleTTSVoiceChange(event) {
    ...
    if (voice) {
        ...
        console.log(`[TTS]   : ${voice.name}`);
        speakText(t('tts.voiceTest', ' '));
    }
}

/**
 *     ,  ,   .
 * @param {string} statKey -   ('str', 'dex', 'int', 'con', 'cha').
 * @returns {number} -  .
 */
function getStatModifier(statKey) {
            ...
            if (effect.effects && Array.isArray(effect.effects)) {
                for (const subEffect of effect.effects) {
                    if (subEffect.action && subEffect.action.stat === statKey && subEffect.action.type === 'modify_stat') {
                        ...
                        if (!isNaN(change)) {
                            totalBonus += change;
                            logMessages.push(` '${effect.name}': ${change > 0 ? '+' : ''}${change}  ${statKey.toUpperCase()}`);
                        }
                    }
                }
            }
    ...
    if (player.skills) {
        for (const skillId in player.skills) {
            ...
            if (skill.skillType && skill.skillType.toLowerCase().includes('') && skill.effectsJSON) {
                try {
                    ...
                    for (const subEffect of parsedEffects) {
                        if (subEffect.action && subEffect.action.stat === statKey && subEffect.action.type === 'modify_stat') {
                            ...
                            if (!isNaN(change)) {
                                totalBonus += change;
                                logMessages.push(` '${skill.name}': ${change > 0 ? '+' : ''}${change}`);
                            }
                        }
                    }
    ...
    if (logMessages.length > 0) {
        console.log(`[getStatModifier]   ${statKey.toUpperCase()}:  ${baseModifier},  ${totalBonus} ->  ${finalModifier}. :`, logMessages.join('; '));
    }
...
function toggleTTS() {
    ...
    const statusMessage = isTTSEnabled ? t('tts.enabled', ' .') : t('tts.disabled', ' .');
...
function updateTTSToggleButton(isEnabled) {
    ...
    if (isEnabled) {
        ...
        toggleTTSButton.title = t('gameInterface.toggleTTSButtonTitlePause', " ");
    } else {
        toggleTTSIcon.classList.remove('fa-comment-slash');
        toggleTTSIcon.classList.add('fa-comment-dots');
        toggleTTSButton.title = t('gameInterface.toggleTTSButtonTitlePlay', " ");
    }
...
async function speakText(text) {
    ...
    try {
        ...
        if (result.success) {
            ...
            console.error("[TTS]  :", result.error);
            showCustomAlert(" TTS:      .   assets/tts/");
        }
    } catch (e) {
        console.error("[TTS]    IPC:", e);
    }
}
...
async function loadItemsReference() {
    ...
    try {
        ...
        console.log(`  (${itemsReferenceData.length} .)    .`);
    } catch (error) {
        console.error(`      :`, error);
        itemsReferenceData = [];
    }
...
async function loadLanguagesConfig() {
    try {
        ...
        console.error("    :", error);
        availableLanguages = {
            [DEFAULT_LANGUAGE]: { name: (DEFAULT_LANGUAGE === 'ru' ? '' : 'Default'), file: `assets/localizations/${DEFAULT_LANGUAGE}.json` }
        };
        populateLanguageSelector();
    }
}
...
async function loadTranslations(langCode) {
    ...
    try {
        ...
        try {
            ...
            console.error(`     ${langCode}  . :`, parseError);
            console.error("---  JSON ,  : ---");
        ...
        console.error(`     ${langCode}:`, fetchError);
        translations = {};
    }
}
...
function applyTranslations() {
    ...
    elements.forEach(el => {
        if (el.id === 'level-info') { // level-info    updateCharacterSheet
            return;
        }
        ...
        // ,     data-i18n (, [placeholder]key.name)
        if (key.startsWith('[')) {
        ...
        if (el.dataset.i18nVariables) {
            try {
                ...
                console.error(`   i18n   "${key}":`, e, el.dataset.i18nVariables);
            }
        }

        let translation = t(key, variables); //  
        ...
        if (translation !== key) { //         
            if (attribute === 'textContent') {
                ...
                //    ,     HTML 
                // (, data-custom-attr),  el.setAttribute .
                //     (, el.value),      ,
                //      updateDynamicUIText   .
                //    (title, placeholder) setAttribute .
                el.setAttribute(attribute, translation);
                // console.warn(`  "${attribute}"          : ${key}.    setAttribute.`);
            }
    ...
    // updateApiKeyStatus(); //    setLanguage
}
...
function updateDynamicUIText() {
    ...
    if (tradeJournalPanelTitle) {
        tradeJournalPanelTitle.textContent = t('gameInterface.tradeJournalPanel.title', null, ' ');
    }
    const portPanelTitle = document.querySelector('.port-panel .panel-toggle > span:first-child');
    if (portPanelTitle) {
        portPanelTitle.textContent = t('gameInterface.portPanel.title', null, '');
    }
    ...
    if (environmentList && environmentList.children.length === 1 && environmentList.firstElementChild.tagName === 'LI') { // 
    ...
    if (globalLocationsList && globalLocationsList.children.length === 1 && globalLocationsList.firstElementChild.tagName === 'LI') {
        ...
        if (Object.keys(globalLocations || {}).filter(key => key !== 'startLocation').length === 0) {
            if (worldLore.startsWith(t('error.prefix', ':')) || worldLore === " ...") {
...
async function initializeApp() {
    ...
    if (failedLoads.length > 0) {
        ...
        if (worldLore.startsWith(':') || Object.keys(globalLocations).length === 0) {
    ...
    // equipmentElements     populateEquipmentUI()
...
function initSettingsUI() {
    ...
    const switchProviderView = (provider) => {
        ...
        switch (provider) {
            ...
            case 'local': modelId = localModelId; break; //  LM Studio   ID
...
function getFriendlyApiErrorMessage(status, rawText) {
    ...
    if (!friendlyText || friendlyText === `apiErrors.${status}`) {
        friendlyText = fallbacks[status] || t('apiErrors.unknown', null, "  API.");
    }
    return `${friendlyText}\n\n[: ${status}] : ${rawText}`;
}
...
async function pingProvider() {
    ...
    resultDiv.innerHTML = '<i class="fas fa-spinner fa-spin" style="color: #f39c12;"></i>  ...';
    btn.disabled = true;
    ...
    try {
        switch (provider) {
            ...
            case 'local':
                ...
                break;
            case 'dummy':
                resultDiv.innerHTML = '<span style="color: #2ecc71;"><i class="fas fa-check"></i>  !</span>';
                btn.disabled = false;
        ...
        if (response.ok) {
            ...
            let shortMsg = t(`apiErrors.${response.status}`, null, ` ${response.status}`);
            resultDiv.innerHTML = `<span style="color: #e74c3c;" title="${errText.replace(/"/g, '&quot;')}"><i class="fas fa-times"></i> ${shortMsg} (: ${response.status})</span>`;
        }
    } catch (e) {
        let shortMsg = e.message.includes('fetch') ? t('apiErrors.network', null, ' ') : e.message;
...
async function fetchModels() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (isGemini && data.models) {
                models = data.models.map(m => {
                    ...
                    return {
                        id: id, name: m.displayName || id, desc: m.description || '  Google Gemini.',
                        type: type, free: false, context: m.inputTokenLimit || null, priceText: ' API (Free Tier)',
                ...
                models = data.data.map(m => {
                    let idLower = (m.id || "").toLowerCase();
                    let isThinking = idLower.includes('r1') || idLower.includes('o1') || idLower.includes('o3') || idLower.includes('thinking') || idLower.includes('reasoning');
                    return { id: m.id, name: m.name || m.id, desc: '', type: 'text', free: false, priceText: '  ', caching: false, thinking: isThinking };
                });
            } else if (Array.isArray(data)) {
                models = data.map(m => {
                    let idLower = (m.id || m.name || "").toLowerCase();
                    let isThinking = idLower.includes('r1') || idLower.includes('o1') || idLower.includes('o3') || idLower.includes('thinking') || idLower.includes('reasoning');
                    return { id: m.id || m.name, name: m.name || m.id, desc: '', type: 'text', free: false, priceText: '  ', caching: false, thinking: isThinking };
                });
            }
        ...
        let shortMsg = e.message.includes('fetch') ? t('apiErrors.network', null, ' ') : e.message;
        if (typeof showCustomAlert === 'function') showCustomAlert("   : " + shortMsg);
    } finally {
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (isGemini && data.models) {
                models = data.models.map(m => {
                    ...
                    return {
                        ...
                        desc: m.description || '  Google Gemini.',
                        type: type,
                        free: false, // FIX:    "",      (Rate Limits)
                        context: m.inputTokenLimit || null,
                        priceText: ' API (Free Tier)' //  
                    };
                });
            } else if (provider === 'openrouter' && data.data) {
                models = data.data.map(m => {
                ...
                models = data.data.map(m => ({ id: m.id, name: m.name || m.id, desc: '', type: 'text', free: false, priceText: '  ' }));
            } else if (Array.isArray(data)) {
                models = data.map(m => ({ id: m.id || m.name, name: m.name || m.id, desc: '', type: 'text', free: false, priceText: '  ' }));
            }
            ...
            if (models.length > 0) {
                ...
                resultDiv.innerHTML = `<span style="color: #f1c40f;"><i class="fas fa-check"></i>  ,    .</span>`;
            }
...
function renderModelList(models) {
    ...
    if (models.length === 0) {
        container.innerHTML = '<div style="padding: 20px; color: #7f8c8d; text-align: center; font-style: italic;">     .</div>';
        return;
    }
    ...
    models.forEach(m => {
        ...
        let badgesHtml = '';
        if (m.free) badgesHtml += `<span class="model-badge badge-free"><i class="fas fa-gift"></i> </span>`;
        ...
        if (m.caching) badgesHtml += `<span class="model-badge badge-caching" title=" Prompt Caching (    )"><i class="fas fa-bolt"></i> Caching</span>`;
        if (m.thinking) badgesHtml += `<span class="model-badge badge-thinking" title="   (Reasoning)"><i class="fas fa-brain"></i> Thinking</span>`;
...
function saveSettings() {
    ...
    updateApiKeyStatus();
    showCustomAlert(t('settingsMenu.apiKeySaved', '  !'));
...
async function loadEnvironmentCommandsGuide(worldId, langCode) {
    try {
        ...
        console.log(`     '${worldId}' (: ${langCode})  .`);
    } catch (error) {
        console.error(`        '${worldId}' (: ${langCode}):`, error);
        environmentCommandsGuideData = t('error.envGuideNotLoadedLang', { worldId: worldId, lang: langCode, error: error.message }, `// :          '${worldId}' (: ${langCode}). ${error.message}`);
    }
}
...
async function loadSkillsReference() {
    try {
        ...
        console.error(`    :`, error);
        skillsReferenceData = t('error.skillsRefNotLoaded', '// :     .');
    }
}
...
async function loadLore(worldId, langCode) {
    if (!worldId) {
        console.error("   : worldId  .");
        worldLore = t('error.worldNotSpecified', ':   .');
        return;
    }
    ...
    try {
        ...
        console.error(`      '${worldId}' (: ${langCode}):`, error);
        worldLore = t('error.loadLoreFailedLang', { worldId: worldId, lang: langCode, error: error.message }, `:       '${worldId}' (: ${langCode}).`);
        globalLocations = {};
        updateMapDisplay();
    }
}
...
async function loadGlobalLocations(worldId, langCode, eraId = getRuntimeDefaultEraId()) {
    ...
    try {
        ...
        console.log(`   '${worldId}' (: ${langCode})  :`, globalLocations);
    } catch (error) {
        console.error(`      '${worldId}' (: ${langCode}):`, error);
        globalLocations = {};
...
function setupEventListeners() {
    ...
    if (gameLog && oocTooltip && oocTooltipContent) {
        gameLog.addEventListener('click', (event) => {
            ...
            if (marker) {
                ...
                oocTooltip.style.top = `${rect.bottom + 5}px`; //   
    ...
    if (testAiPlayerBtn) {
        testAiPlayerBtn.addEventListener('click', async () => {
            ...
            try {
                const response = await performAiPlayerFetch("  .  '  ,   .'", [], mod, " .");
                resultDiv.innerHTML = `<span style="color: #2ecc71;"> :</span><br>${response}`;
            } catch (e) {
                resultDiv.innerHTML = `<span style="color: #e74c3c;"> :</span><br>${e.message}`;
            } finally {
...
function startNewGameSetup() {
    ...
    if (keyIsMissing) {
        ...
        showCustomAlert(t('error.apiKeyNeededForProvider', { provider: providerName }, `    API    ${providerName}. ,    .`));
        settingsReturnScreen = 'main-menu'; // ,     
        setActiveScreen('settings-menu');
        return;
    }
    ...
    //   ,       
    if (worldLore.startsWith(t('error.prefix', ":")) || Object.keys(globalLocations).length === 0) {
        alert(t('error.worldLoadFailed', { worldId: DEFAULT_WORLD_ID }, `        (${DEFAULT_WORLD_ID}).   (F12)   .`));
        return;
    }

    console.log(`       : ${DEFAULT_WORLD_ID}`);
    ...
    const proceedToNarrator = async () => {
        ...
        tempPlayer = {
            ...
            bankAccount: { deposit: 0, loan: 0, loanDays: 0 },
                        inventory: {}, // Legacy
            container_backpack: null, // :      C++ 
            container_equipment: null, // :      C++ 
            echoMemory: { items: [], maxItems: ECHO_MEMORY_MAX_ITEMS, version: 1 },
gmNotes: { "Main_Plot": " .     ." },
            memoryArchives: {},
            archiveSummaries: {},
            factionData: { global: t('factions.global', null, '') },
            location: t('world.generatingStartLocation', "  ..."),
            nexusData: {},
            worldEvents: [],
            quests: {},
            skills: {},
        ...
        console.log("     '" + selectedEra + "',    :", tempPlayer);
...
async function finalizeWorldSetupAndStart() {
    ...
    if (preloadedWorldData) {
        ...
        if (window.electronAPI && window.electronAPI.nexusBootstrap) {
            ...
            const loadingText = document.getElementById('loading-text');
            if (loadingText) loadingText.textContent = `  (${bootstrapDays} .)...`;
            console.log(`[Nexus]  Bootstrap  ${bootstrapDays} ...`);
        ...
        if (enableWorldSim) {
            ...
            const loadingText = document.getElementById('loading-text');
            if (loadingText) loadingText.textContent = t('loadingScreen.finalizing', null, '...');
        }
    ...
    //   :   ,    stdin.
    // syncState  stdin   (1.5MB+ JSON  64KB pipe buffer  timeout).
    //  :    ,      loadWorldFile.
        ...
        try {
            ...
            if (writeRes.status === 'ok' && writeRes.path) {
                ...
                if (loadRes.status === 'ok') {
                    console.log('[Nexus]    :', loadRes.message);
                } else {
                    console.warn('[Nexus] loadWorldFile  :', loadRes.message || loadRes.error || 'unknown error');
                }
            } else {
                console.warn('[Nexus]     :', writeRes.message);
            }
        } catch (err) {
            console.warn('[Nexus]   :', err.message || err);
        }
    }
    ...
    console.log(`     '${player.era}': ${initialPromptFile}`);
    const initialPromptTemplate = await loadPromptFromFile(initialPromptFile);

    if (initialPromptTemplate.startsWith(':')) {
    ...
    if (Array.isArray(itemsReferenceData) && itemsReferenceData.length > 0) {
        ...
        } catch (e) { console.error("  itemsReferenceData   :", e); }
    }
    ...
    if (typeof World !== 'undefined' && World && startRegionId && World.regions[startRegionId]) {
        let r = World.regions[startRegionId];
        dynamicContextStr += `\n===   ( ) ===\n`;
        let seasonName = r.current_season === 'spring' ? '' : (r.current_season === 'summer' ? '' : (r.current_season === 'autumn' ? '' : ''));
        dynamicContextStr += `[ ] : ${r.name}. : ${seasonName}. : ${r.weather || ""}\n`;
        ...
        let activeDisasters = (World.map && World.map.disasters) ? World.map.disasters.filter(d => d.days_active > 0 && d.affected_regions.includes(startRegionId)) : [];
        if (activeDisasters.length > 0) {
            dynamicContextStr += `\n[   ]:     : ${activeDisasters.map(d => d.type).join(', ')}!       !\n`;
        }
    ...
    if (typeof World !== 'undefined' && World && World.news && World.news.length > 0) {
        ...
        let recentNews = World.news
            .map(n => ({ ...n, daysOld: Math.max(0, currentDay - (n.day || 0)) }))
            .filter(n => n.daysOld <= 720) //   2 
            .filter(n => ['war', 'disaster', 'politics'].includes(n.category)) //    
            .sort((a, b) => a.daysOld - b.daysOld)
            .slice(0, 15);
            
        if (recentNews.length > 0) {
            dynamicContextStr += `\n===   ( -) ===\n`;
            dynamicContextStr += recentNews.map(n => `[${n.daysOld} . , : ${n.location}] ${parseLocString(n.text)}`).join("\n");
            dynamicContextStr += `\n          (   ,   ,  )!\n`;
        }
    }
    ...
    const genBackstoryText = player.generateBackstory ? "TRUE (      setPlayerDescription)" : "FALSE";
...
function updateTimeDisplay() {
    ...
    if (timeInfo && player && player.gameTime) {
        ...
        timeInfo.innerHTML = `${icon} ${gt.day} ${mName}, ${gt.year} . | ${hh}:${mm}`;
    }
}
...
function getStartingInventory(playerClass) {
    ...
    for (const itemAiId in startingItemConfig) {
        ...
        if (itemRef) {
            ...
            console.warn(`[getStartingInventory]       ID: ${itemAiId}.   .`);
        }
    }
    return inventory;
}
...
function levelUp() {
    ...
    const _prevLevel = player.stats.level;
        ...
        player.stats.xp = Math.max(0, excessXp); //  
    ...
    if (levelsGainedThisCycle > 0) {
        ...
        generateWorldNews(` ${player.name}  ${player.stats.level} !`, player.location || "global", 3, 'misc');
        updateCharacterSheet(); //  ,   "+"
    }
}
...
function updateNexusDisplay() {
    ...
    const actualItems = nexusData.filter(item => {
        ...
        if (item.category === 'World_Event' || item.category === ' ' || item.id.startsWith('event_')) return false;
        return true;
    });

    if (actualItems.length === 0) {
        nexusList.innerHTML = `<li data-i18n="gameInterface.nexusPanel.empty">${t('gameInterface.nexusPanel.empty', ' ')}</li>`;
        return;
    }

    //     
    const groupedData = actualItems.reduce((acc, item) => {
        const category = item.category || t('gameInterface.nexusPanel.defaultCategory', '');
        if (!acc[category]) {
            acc[category] = [];
        }
    ...
    for (const category of sortedCategories) {
        ...
        groupedData[category].sort((a, b) => a.name.localeCompare(b.name, currentLanguage)).forEach(item => {
            ...
            li.addEventListener('mouseenter', (e) => {
                ...
                const content = `<div style="color:#5d4a36; font-style:italic; margin-bottom: 5px; border-bottom: 1px solid rgba(0,0,0,0.1); padding-bottom: 3px;">: ${item.category || ''}</div>
            ...
            switch (item.displayType) {
                case 'boolean':
                    valueDisplay = item.value === 'true'
                        ? t('gameInterface.nexusPanel.boolTrue', '')
                        : t('gameInterface.nexusPanel.boolFalse', '');
                    break;
                case 'numeric':
...
async function loadPredefinedEffects() {
    try {
        ...
        console.log(`  (${Object.keys(predefinedStatusEffects).length} .)  .`);
    } catch (error) {
        console.error(` :       :`, error);
        predefinedStatusEffects = {};
                ...
                for (const subEffect of parsedEffects) {
                    if (subEffect.action && subEffect.action.type === 'modify_stat') {
                        ...
                        if (player.stats[stat] !== undefined && !isNaN(change)) {
                            ...
                            addLogMessage(` '${nexusItem.name}'   ! (${stat.toUpperCase()} ${change > 0 ? '+' : ''}${change})`, 'level-up');
                            addCalculationMessage(`[NEXUS_AUTO]  '${key}' . ${stat.toUpperCase()} ${change > 0 ? '+' : ''}${change}.`);
                        }
                    }
                }
...
function updateDiceLogDisplay() {
    ...
    history.forEach(entry => {
        ...
        let html = `<div style="color: #f39c12; font-size: 0.85em; margin-bottom: 4px; font-weight: bold;"> ${entry.turn}</div>`;
...
async function initializeGameInterface() {
    ...
    window.advanceJourney = async function () {
        ...
        if (player.currentJourney.currentPoint > player.currentJourney.points) {
            ...
            userInput.value = `[SYSTEM:  .    ${player.currentJourney.destination}.      endJourney]`;
            handleUserInput();
            return;
        }
        ...
        if (options.length === 0) {
            addLogMessage(`* ${player.currentJourney.currentPoint}   .*`, "gm-message");
            return;
        }
        ...
        //   
        addLogMessage(`**[  ${player.currentJourney.currentPoint}/${player.currentJourney.points}]**\n${selectedEvent.text}`, "gm-message");
        ...
        if (selectedEvent.type === 'combat') {
            ...
            updateCharacterSheet();
            addLogMessage(`(( :   .  : ${selectedEvent.stat.toUpperCase()} (: ${selectedEvent.dc}).    ! ))`, "system-message");
        } else if (selectedEvent.type === getInventoryLootRuntimeConfig().event_type) {
            if (selectedEvent.itemId) {
                executeCommand('addItem', { aiIdentifier: selectedEvent.itemId, name: selectedEvent.itemName || "", quantity: selectedEvent.amount ?? requireRuntimeNumber(getInventoryLootRuntimeConfig().default_quantity, 'gameplay_runtime.inventory_loot.default_quantity') });
            }
        }
    };
    ...
    updateQuestList();
    updateSkillsDisplay();
    updateMapDisplay(); //    renderVisualMap
    updateWorldChroniclesDisplay();
    updateTradeJournalDisplay();
    updatePortPanel();

    //   (CityGen)    Nexus Cartographer
...
function updateCharacterSheet() {
    ...
    if (player && player.travel && player.travel.active) {
        ...
        if (journeyContainer) {
            ...
            journeyDest.textContent = ` : ${player.travel.destinationName}`;
            journeyProgressText.textContent = `${player.travel.elapsedHours} / ${player.travel.totalHours} .`;
                ...
                if (journeyEventArea) {
                    ...
                    player.travel.currentEvents.forEach(ev => {
                        ...
                        htmlText += `<div class="journey-event-row">
                                        <div class="journey-event-text-container"><strong>[]</strong> ${description}</div>`;
                        if (ev.can_interact) {
                            htmlText += `<div class="journey-event-btn-container">
                                            <button class="travel-action-btn" onclick="LivingRoads.interact('${ev.object_type}', '${ev.sim_object_id}', '${safeDesc}')"><i class="fas fa-search"></i> </button>
                                         </div>`;
                        }
                        htmlText += `</div>`;
                    });
                    ...
                    if (journeyEventActions) {
                        ...
                        if (hasCriticalEvent) {
                            journeyEventActions.innerHTML = `<div style="text-align:center; color:#e74c3c; font-size:0.8em; padding:5px;"><i class="fas fa-exclamation-triangle"></i>     .  .</div>`;
                        } else {
                            journeyEventActions.innerHTML = `<button class="travel-action-btn btn-continue" style="width: 100%; margin: 0;" onclick="LivingRoads.resume()"><i class="fas fa-shoe-prints"></i>  </button>`;
                        }
                    }
                }
            } else {
            ...
            if (travelControls && travelControls.style.display === 'flex') {
                if (travelPauseBtn) {
                    ...
                    travelPauseBtn.title = player.travel.paused ? ' ' : ' ()';
                }
        ...
        if ((player.currentCombat && player.currentCombat.isActive) || (player.travel.paused && (!player.travel.currentEvents || player.travel.currentEvents.length === 0))) {
            ...
            if (userInput) {
                ...
                userInput.placeholder = (player.currentCombat && player.currentCombat.isActive) ? "     ?" : " .  ?";
            }
        } else {
            if (sendButton) sendButton.style.display = 'none';
            if (userInput) {
                userInput.disabled = true;
                userInput.placeholder = LivingRoads.isGeneratingHour ? " ..." : (player.travel.currentEvents && player.travel.currentEvents.length > 0 ? "" : "  ... ( )");
            }
        }
        ...
        if (journeyContainer) {
            ...
            //     
            if (player.currentCombat && player.currentCombat.isActive) {
                journeyDest.innerHTML = ` : ${player.currentJourney.destination} <span style="color: #e74c3c;">[!]</span>`;
            } else {
                journeyDest.textContent = ` : ${player.currentJourney.destination}`;
            }
    ...
    charNameDisplay.onclick = () => {
        ...
        if (modal && content) {
            ...
            content.textContent = player.description || "    ...";
            modal.style.display = 'flex';
            setTimeout(() => modal.classList.add('visible'), 10);
        }
    };
    ...
    if (player.class === 'mage') {
        ...
        statsToUpdate.forEach(statKey => {
            ...
            statLine.onmouseenter = (e) => {
                let content = `<div style="color:#1a110a; font-size: 1.1em; margin-bottom:5px; border-bottom: 1px solid rgba(0,0,0,0.2); padding-bottom: 3px;"> : <b>${effectiveStats[statKey]}</b></div>`;
                content += `<div style="color:#2c1e14; margin-bottom:5px;"> : <b>${base}</b></div>`;
...
function updateInventoryDisplay() {
    ...
    const filteredItems = allItems.filter(item => {
        ...
        if (currentInventoryFilter === 'potion' && (iType === 'potion' || iType === '' || iType === 'consumable')) return true;
        if (currentInventoryFilter === 'weapon' && (iType === 'weapon' || iType === '')) return true;
        if (currentInventoryFilter === 'armor' && (iType === 'armor' || iType === '')) return true;
    ...
    if (filteredItems.length === 0) {
        ...
        filteredItems.forEach(item => {
            ...
            const itemName = props.name || item.prototype_id || ' ';
            let rarityClass = props.rarity ? props.rarity.toLowerCase().replace(/[^a-z-0-9]/g, '-') : '';
            ...
            if (isTransport) {
                ...
                mountBtn.textContent = isMounted ? t('transport.dismount', '') : t('transport.mount', '');
...
function showItemExamineModal(item) {
    ...
    document.getElementById('examine-title').textContent = parseLocString(item.name) || ' ';
    ...
    document.getElementById('examine-desc').textContent = parseLocString(item.description) || ' .';

    let statsHtml = '';
    if (item.effects && item.effects.length > 0) {
        statsHtml += `<div><strong>:</strong> ${item.effects.map(e => `${e.stat.toUpperCase()} ${e.change > 0 ? '+' : ''}${e.change}`).join(', ')}</div>`;
    }
    statsHtml += `<div><strong>:</strong> ${item.value || 0} </div>`;
    document.getElementById('examine-stats').innerHTML = statsHtml || ' ';

    const historyEl = document.getElementById('examine-history-content');
    if (item.history && item.history.length > 0) {
        historyEl.innerHTML = item.history.map(h => `<div style="margin-bottom: 4px; border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 2px;"><span style="color:#f39c12;">[ ${h.day}]</span> ${parseLocString(h.event)}</div>`).join('');
    } else {
        historyEl.innerHTML = '<div style="font-style:italic;">      ...</div>';
    }
...
function updateStatusEffectsDisplay() {
    ...
    if (effects.length === 0) {
        statusEffectsList.innerHTML = `<li data-i18n="gameInterface.statusEffectsPanel.empty">${t('gameInterface.statusEffectsPanel.empty', '  ')}</li>`;
    } else {
        effects.sort((a, b) => (a.name || '').localeCompare(b.name || ''));
        effects.forEach(effect => {
            const li = document.createElement('li');
            li.classList.add('status-effect-item');
            li.title = effect.description || t('gameInterface.statusEffectsPanel.noDescription', '  .');
...
function updateQuestList() {
    ...
    if (activeQuests.length === 0) {
        ...
        activeQuests.forEach(quest => {
            ...
            const title = quest.title || t('quests.defaultTitle', null, ' ');
            const objective = quest.objective || '?';
            const description = quest.description || t('quests.noDescription', null, ' ');
            let rawReward = quest.reward;
            let rawIssuer = quest.issuer;
            let rewardValue = t('quests.unknown', null, '');
            ...
            let issuerValue = t('quests.unknown', null, '');
...
function updateSkillsDisplay() {
    window.activatePlayerSkill = function (skillId) {
        ...
        if (costType.includes('mp') || costType.includes('')) {
            if (player.stats.mana < costVal) { showCustomAlert(" !"); return; }
        } else if (costType.includes('hp') || costType.includes('') || costType.includes('stamina') || costType.includes('')) {
            if (player.stats.hp <= costVal) { showCustomAlert(" /!"); return; }
        }
    ...
    function createSkillBadge(skillId, skillName, skillEffect) {
        ...
        for (let b of existing) {
            if (b.dataset.skillId === skillId) return; //  
        }

        const badge = document.createElement('div');
        badge.className = 'roll-badge skill-badge';
        badge.dataset.skillId = skillId;
        badge.dataset.resultText = `[SYSTEM_MECHANIC:   | : ${skillName} | : ${skillEffect}]`;
    ...
    if (learnedSkills.length === 0) {
        ...
        learnedSkills.forEach(skill => {
            ...
            li.title = skill.description || t('skills.noDescription', null, ' ');

            let detailsHTML = '';
            if (skill.damage && String(skill.damage).toLowerCase() !== '') {
                detailsHTML += `<span><strong>${t('skills.damageLabel', '')}:</strong> ${skill.damage}</span>`;
            }
            if (skill.cost && skill.costType && String(skill.costType).toLowerCase() !== '') {
                detailsHTML += `<span><strong>${t('skills.costLabel', '')}:</strong> ${skill.cost} ${skill.costType}</span>`;
            } else if (skill.cost && String(skill.cost).toLowerCase() !== '0' && String(skill.cost).toLowerCase() !== '') {
                detailsHTML += `<span><strong>${t('skills.costLabel', '')}:</strong> ${skill.cost}</span>`;
            }
            if (skill.duration && String(skill.duration).toLowerCase() !== '') {
                detailsHTML += `<span><strong>${t('skills.durationLabel', '.')}:</strong> ${skill.duration}</span>`;
            }
            if (skill.cooldown && String(skill.cooldown).toLowerCase() !== '') {
                detailsHTML += `<span><strong>${t('skills.cooldownLabel', '.')}:</strong> ${skill.cooldown}</span>`;
            }
            if (skill.skillType && String(skill.skillType).toLowerCase() !== '') {
                detailsHTML += `<span><strong>${t('skills.typeLabel', '')}:</strong> ${skill.skillType}</span>`;
            }

            let effectDisplay = skill.effect || '';
            if (effectDisplay.toLowerCase() === '') effectDisplay = '';

            let cdText = '';
            let isUsable = false;
            if (skill.skillType && skill.skillType.toLowerCase().includes('')) {
                isUsable = true;
                if (skill.currentCooldown > 0) {
                    cdText = `<span style="color:#e74c3c; font-weight:bold; margin-left:10px; font-size:0.85em;">(: ${skill.currentCooldown} .)</span>`;
                }
            }
            ...
            li.innerHTML = `
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:5px;">
                    ...
                    ${isUsable && (!skill.currentCooldown || skill.currentCooldown <= 0) ? `<button class="use-skill-btn" data-id="${skill.id}">${t('gameInterface.skills.use', null, '')}</button>` : ''}
                </div>
                <span class="skill-description">${skill.description || ''}</span>
                ${detailsHTML ? `<div class="skill-details">${detailsHTML}</div>` : ''}
                ${effectDisplay ? `<div class="skill-effect"><strong>${t('skills.effectLabel', '')}:</strong> ${effectDisplay}</div>` : ''}
            `;
...
function updateWorldChroniclesDisplay() {
        ...
        filterContainer.outerHTML = `
            <div class="chronicle-ui-container" id="chronicle-ui-container">
                <div class="chronicle-filter-row">
                    ...
                    <button class="c-filter-btn ${currentChronicleFilter === 'war' ? 'active' : ''}" data-filter="war"> ${t('extraLoc.chronicles.wars')}</button>
                    <button class="c-filter-btn ${currentChronicleFilter === 'disaster' ? 'active' : ''}" data-filter="disaster"> ${t('extraLoc.chronicles.disasters')}</button>
                    ...
                    <button class="c-filter-btn ${currentChronicleFilter === 'market' ? 'active' : ''}" data-filter="market"> ${t('extraLoc.chronicles.market')}</button>
                    ...
                    <button class="c-filter-btn ${currentChronicleFilter === 'politics' ? 'active' : ''}" data-filter="politics"> </button>
                    <button class="c-filter-btn ${currentChronicleFilter === 'misc' ? 'active' : ''}" data-filter="misc"> ${t('extraLoc.chronicles.rumors')}</button>
                </div>
                <div class="chronicle-filter-row">
    ...
    let events = Object.values(player.nexusData || {}).filter(item => 
        item.category === 'World_Event' || item.category === ' ' || item.id.startsWith('event_')
    );
...
function renderChroniclePage(page) {
    ...
    chunk.forEach(item => {
        ...
        if (item.type === 'nexus') {
            const ev = item.data;
            const isActive = (ev.value === '' || ev.value === 'active' || ev.value === '' || ev.value === 'ACTIVE');
            ...
            if (news.category === 'politics') { icon = '<i class="fas fa-landmark"></i>'; catName = t('extraLoc.chronicles.politics', null, ''); color = '#8e44ad'; }
...
function getResourceIcon(res) {
    ...
    const fallback = {"wheat":"","meat":"","fish":"","wood":"","iron_ore":"","gold_ore":"","cotton":"","herbs":"","salt":"","stone":""};
...
function updateMapDisplay() {
    if (window.Cartographer) {
        Cartographer.fetchMapData().then(() => {
            ...
            //     requestAnimationFrame  Cartographer
        });
    }
}
...
function updateEnvironmentPanel() {
    ...
    if (entities.length === 0) {
        ...
        entities.forEach(entity => {
            ...
            nameSpan.textContent = entity.name || entity.id || t('gameInterface.environmentPanel.unknownEntity', ' ');
            ...
            li.dataset.tooltipData = JSON.stringify({
                ...
                savings: savings,
                description: entity.description || t('gameInterface.environmentPanel.noDescription', '  .'),
                hp: entity.stats?.hp,
            ...
            const isMerchant = entity.type === 'npc' && entity.traits &&
                ['merchant', 'trader', 'peddler', '', ''].some(t =>
...
function createItemTooltip(event, item) {
    ...
    if (item.effects && item.effects.length > 0) {
        effectsHtml = `<div style="margin-top:8px; border-top:1px dashed #2c1e14; padding-top:5px; font-weight:bold;">
            : ${item.effects.map(e => `${e.stat.toUpperCase()} ${e.change > 0 ? '+' : ''}${e.change}`).join(', ')}
        </div>`;
    }

    let historyHtml = '';
    if (item.history && item.history.length > 0) {
        const historyItems = item.history.slice(-3).map(h => `[ ${h.day}] ${parseLocString(h.event)}`).join('<br>');
        historyHtml = `<div style="margin-top:8px; border-top:1px dashed #2c1e14; padding-top:5px; font-size:0.85em; color:#d35400;">
            <strong> :</strong><br>${historyItems}
        </div>`;
    }
    ...
    itemTooltipElement.innerHTML = `
        ...
        <div class="item-card-body">
            <span class="item-card-rarity" style="color: ${rarityColor}">${item.rarity || ''}</span>
            <div style="font-style:italic;">${parseLocString(item.description)}</div>
            ${effectsHtml}
            ${historyHtml}
            <div style="margin-top:8px; font-size:0.85em; text-align:right; opacity:0.8;"> : ${item.value || 0}</div>
        </div>
    `;
...
function getRarityColor(r) {
    ...
    if (s.includes('')) return '#1eff00';
    if (s.includes('')) return '#0070dd';
    if (s.includes('')) return '#a335ee';
    if (s.includes('')) return '#ff8000';
    return '#5d4a36';
}
...
function showEntityTooltip(event) {
    ...
    let statsHtml = '';
    if (data.str !== undefined) statsHtml += `<p><span class="stat-label">${t('gameInterface.characterPanel.str', ' ')}:</span> <span class="stat-value">${data.str}</span></p>`;
    if (data.dex !== undefined) statsHtml += `<p><span class="stat-label">${t('gameInterface.characterPanel.dex', ' ')}:</span> <span class="stat-value">${data.dex}</span></p>`;
    if (data.con !== undefined) statsHtml += `<p><span class="stat-label">${t('gameInterface.characterPanel.con', ' ')}:</span> <span class="stat-value">${data.con}</span></p>`;
    if (data.int !== undefined) statsHtml += `<p><span class="stat-label">${t('gameInterface.characterPanel.int', ' ')}:</span> <span class="stat-value">${data.int}</span></p>`;
    if (allowNSFW && data.lust !== undefined) statsHtml += `<p><span class="stat-label" style="color:#e91e63;"> :</span> <span class="stat-value" style="color:#e91e63;">${data.lust}%</span></p>`;
    ...
    if (data.hp !== undefined && data.maxHp !== undefined && data.maxHp > 0) {
        ...
        if (!data.isHostile) {
            if (data.type.toLowerCase() === t('gameInterface.environmentPanel.entityTypeNPC', '').toLowerCase()) {
        ...
        `;
        healthText = `<p><strong>${t('gameInterface.environmentPanel.tooltip.health', '')}:</strong> <span class="stat-value">${data.hp} / ${data.maxHp}</span></p>`;
    }


    let traitsHtml = '';
    if (data.traits && data.traits.length > 0) {
        traitsHtml = `<p><strong style="color: #9b59b6;">:</strong> <span style="color: #ecf0f1; font-style: italic;">${data.traits.join(', ')}</span></p>`;
    }

    let econHtml = '';
    if (data.profession_type && data.profession_type !== 'none') {
        const profMap = { 'farmer': '', 'artisan': '', 'merchant': '', 'innkeeper': '', 'ruler': '', 'cleric': '', 'mage': '', 'mercenary': '' };
        econHtml = `<p><strong style="color: #2ecc71;">:</strong> <span style="color: #ecf0f1;">${profMap[data.profession_type] || data.profession_type}</span> | <strong style="color: #f1c40f;">:</strong> ${data.savings} .</p>`;
    }
    ...
    if (typeof World !== 'undefined' && World && World.npcs && World.npcs[data.id] && World.npcs[data.id].wounds) {
        ...
        if (wounds.length > 0) {
            const wList = wounds.map(w => `${w.type} (: ${w.severity})`).join(', ');
            woundsHtml = `<p><strong style="color: #e74c3c;">:</strong> <span style="color: #ffcccc;">${wList}</span></p>`;
        }
    }

    entityTooltip.innerHTML = `
        <h4>${data.name}</h4>
        <p><strong>${t('gameInterface.environmentPanel.tooltip.type', '')}:</strong> ${data.type}</p>
        ${econHtml}
        ${traitsHtml}
        ${woundsHtml}
        ${healthText}
...
function moveEntityTooltip(event) {
    if (entityTooltip && entityTooltip.style.display === 'block') {
        ...
        //     
        if (newX + tooltipRect.width > window.innerWidth - 10) { // 10px   
...
function showNPCDetailsModal(npcId) {
    ...
    if (rel.sexualHistory && rel.sexualHistory.length > 0) {
        const historyItems = rel.sexualHistory.slice(-5).reverse().map(h => {
            const typeLabel = h.type === 'consensual' ? ' ' :
                             h.type === 'forced' ? ' ' :
                             h.type === 'seduction' ? ' ' : h.type;
            return `
                <div style="background: rgba(0,0,0,0.3); padding: 8px; border-radius: 6px; margin-bottom: 8px; border-left: 3px solid #e74c3c;">
                    <div style="font-size: 0.9em;"><strong> ${h.day}</strong>  ${h.location}</div>
        ...
        historyHtml = `
            <div style="margin-top: 20px; padding-top: 20px; border-top: 2px solid #5d4a36;">
                <h3 style="margin: 0 0 15px 0; color: #e74c3c;">   </h3>
                ${historyItems}
                ${rel.sexualHistory.length > 5 ? `<div style="text-align: center; color: #95a5a6; font-size: 0.85em; margin-top: 10px;">...  ${rel.sexualHistory.length - 5} </div>` : ''}
            </div>
        `;
    } else {
        historyHtml = `
            <div style="margin-top: 20px; padding-top: 20px; border-top: 2px solid #5d4a36;">
                <h3 style="margin: 0 0 10px 0; color: #95a5a6;">   </h3>
                <div style="text-align: center; color: #7f8c8d; font-style: italic; padding: 20px;">      </div>
            </div>
        `;
    }
    ...
    modalContent.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
            ...
            <button id="close-npc-modal" style="background: #e74c3c; border: none; color: white; padding: 8px 15px; border-radius: 6px; cursor: pointer; font-weight: bold;"> </button>
        </div>

        <div style="background: rgba(0,0,0,0.3); padding: 15px; border-radius: 8px; margin-bottom: 20px;">
            <div style="font-style: italic; color: #bdc3c7;">${npc.description || ' '}</div>
        </div>

        <h3 style="margin: 20px 0 15px 0; color: #d4af37;">   </h3>

        ${createProgressBar('  (Affection)', rel.affection, -100, 100)}
        ${createProgressBar('  (Attraction)', rel.attraction, 0, 100)}
        ${createProgressBar('  (Trust)', rel.trust, 0, 100)}
        ${createProgressBar('  (Intimacy)', rel.intimacy, 0, 100)}
...
function updateEroticJournal() {
    ...
    if (player.eroticStats && player.eroticStats.totalScenes > 0) {
        ...
        let fetishesStr = [];
        if (stats.fetishes.anal > 0) fetishesStr.push(` (${stats.fetishes.anal})`);
        if (stats.fetishes.oral > 0) fetishesStr.push(` (${stats.fetishes.oral})`);
        if (stats.fetishes.bdsm > 0) fetishesStr.push(` (${stats.fetishes.bdsm})`);
        if (stats.fetishes.group > 0) fetishesStr.push(` (${stats.fetishes.group})`);
        let fetishesDisplay = fetishesStr.length > 0 ? fetishesStr.join(', ') : "";

        statsDiv.innerHTML = `
            <strong style="color: #e74c3c;"> :</strong><br>
             : ${stats.totalScenes} |  : ${stats.partners.length}<br>
            : ${stats.types.consensual || 0} | : ${stats.types.seduction || 0} | : ${stats.types.forced || 0}<br>
             : ${favLoc} (${maxLoc} )<br>
            : ${fetishesDisplay}
        `;
    ...
    if (!player.eroticJournal || player.eroticJournal.length === 0) {
        journalList.innerHTML = `<li style="color: #7f8c8d; font-style: italic; text-align: center; padding: 20px;" data-i18n="gameInterface.eroticJournalPanel.empty">${t('gameInterface.eroticJournalPanel.empty', '')}</li>`;
        return;
    }
...
function showEroticSceneModal(sceneId) {
    ...
    const typeLabel = scene.type === 'consensual' ? t('gameInterface.eroticJournalPanel.typeConsensual', ' ') :
                     scene.type === 'forced' ? t('gameInterface.eroticJournalPanel.typeForced', ' ') :
                     scene.type === 'seduction' ? t('gameInterface.eroticJournalPanel.typeSeduction', ' ') : scene.type;
    ...
    modalContent.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; border-bottom: 2px solid #e74c3c; padding-bottom: 15px;">
            ...
            <button id="close-scene-modal" style="background: #c0392b; border: none; color: white; padding: 8px 15px; border-radius: 6px; cursor: pointer; font-weight: bold;"> ${t('gameInterface.eroticJournalPanel.closeModal', '')}</button>
        </div>

        <div style="background: rgba(0,0,0,0.3); padding: 15px; border-radius: 8px; margin-bottom: 20px;">
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; font-size: 0.9em;">
                <div><strong> ${t('gameInterface.eroticJournalPanel.sceneDay', '')}:</strong> ${scene.day}</div>
                <div><strong> ${t('gameInterface.eroticJournalPanel.sceneLocation', '')}:</strong> ${scene.location}</div>
                <div><strong> ${t('gameInterface.eroticJournalPanel.sceneType', '')}:</strong> ${typeLabel}</div>
                <div><strong> :</strong> ${intensityLabel}</div>
            </div>
        </div>
...
function clearEroticJournal() {
    ...
    const confirmMessage = t('gameInterface.eroticJournalPanel.confirmClear', ' ,        ?   .');
    ...
    try {
        ...
        if (typeof window.getRuntimePrompt === 'function') {
            ...
            if (runtimePrompt && typeof runtimePrompt.content === 'string') {
                console.log(`    runtime registry: ${filePath}`);
...
function addLogMessage(message, type = "gm-message", isRestoring = false, imagePrompt = "", savedImageBase64 = null) {
    if (!gameLog) return;
    message = parseLocString(message); // -
        ...
        if (type === 'world-event') {
            ...
            cleanHtml = `
                <div class="world-event-card">
                    <div class="world-event-header"><i class="fas fa-globe-europe"></i>   </div>
            ...
            let processedHtml = rawHtml.replace(rpRegex, (match, ooc, dialogue, dialogueContent, action, actionContent) => {
                if (ooc) {
                    ...
                    return `<span class="ooc-marker" data-ooc-text="${oocText}" title="${t('gameInterface.log.oocTooltip', 'OOC ')}">OOC</span>`;
                }
    ...
    if (category === 'world-event') {
        ...
        if (category === 'system') {
            ...
            hintDiv.innerHTML = '<i class="fas fa-chevron-down"></i>  ';
            ...
            bubble.addEventListener('click', (e) => {
                ...
                hintDiv.innerHTML = isCollapsed ? `<i class="fas fa-chevron-down"></i> ${t('gameInterface.log.systemLogs', null, ' ')}` : `<i class="fas fa-chevron-up"></i> ${t('gameInterface.log.collapse', null, '')}`;
            });
        } else {
    ...
    if (textToSpeak && textToSpeak.trim() !== '' && category !== 'system') {
        if (!bubbleElement.querySelector('.tts-controls-wrapper')) {
            ...
            ttsBtn.title = t('gameInterface.tooltip.speak', null, '');
            ttsBtn.dataset.state = 'idle';

            const stopBtn = document.createElement('button');
            stopBtn.className = 'tts-stop-btn';
            stopBtn.innerHTML = '<i class="fas fa-stop"></i>';
            stopBtn.title = t('gameInterface.tooltip.stop', null, '');
            ...
            const attachImageToChat = (srcBase64) => {
                ...
                watermark.textContent = t('gameInterface.tooltip.etherealSnapshot', null, ' ');
            ...
            if (savedImageBase64) {
                ...
                imgBtn.title = t('gameInterface.tooltip.visualize', null, ' ');
...
async function loadCombatSystemRules() {
    try {
        ...
        console.error(`     :`, error);
        combatSystemRulesData = "// :      .    .";
    }
}

// ---     () ---

function showLoadingScreen(textKey = 'loadingScreen.generatingWorld', fallbackText = ' ...') {
 ...
 *      Gemini API.
 *    ,  ,     .
 * @param {string} promptTextForAI -        .
 * @param {boolean} [isInitialPrompt=false] - , ,        .
 * @param {boolean} [isDiceRollResponse=false] - , ,        .
 * @param {Array<string>} [expiredEffects=[]] -   -,     .
 */
...
async function handleUserInput() {
    ...
    if (DEBUG_MODE && text.startsWith('/dev ')) {
        ...
        if (cmd === 'turn') {
            ...
            addLogMessage(`[DEV]    : ${player.stats.turnCount}`, "system-message");
            updateCharacterSheet();
        }
        else if (cmd === 'addmem') {
            if (!player.gmNotes) player.gmNotes = {};
            player.gmNotes[`Test_Data_${Date.now()}`] = "       .        .";
            addLogMessage(`[DEV]   GM   .  F4,  .`, "system-message");
    ...
    updateWorldSimDebugDisplay();
            ...
            addLogMessage(`[DEV]    .`, "system-message");
        }
        else if (cmd === 'skip') {
            const skipAmount = val || 1;
            player.stats.turnCount += skipAmount;
            addLogMessage(`[DEV]  : ${skipAmount}.  : ${player.stats.turnCount}
 :  ${player.gameTime.day}, ${player.gameTime.hour < 10 ? '0'+player.gameTime.hour : player.gameTime.hour}:${player.gameTime.minute < 10 ? '0'+player.gameTime.minute : player.gameTime.minute} (${player.timeOfDay})
[   ]: ${(() => {
    ...
    return d > 0 ? `${d} .  ${h} .` : `${h} .`;
            ...
            addLogMessage(`[DEV]  . : /dev turn [], /dev addmem, /dev killall, /dev skip [], /dev test`, "system-message");
        }
        ...
        badges.forEach(badge => {
            ...
            if (badge.classList.contains('skill-badge')) {
                ...
                if (skill) {
                    let costVal = parseInt(skill.cost) || 0;
                    let costType = (skill.costType || '').toLowerCase();
                    if (costType.includes('mp') || costType.includes('')) player.stats.mana -= costVal;
                    else if (costType.includes('hp') || costType.includes('')) damagePlayerHP(costVal);
    ...
    if (rollResultsArray.length > 0) {
        ...
        addCalculationMessage(`[]  : ${rollResultsArray.length}`);
    ...
    if (rollsContainer && rollsContainer.children.length > 0) {
        let rollsHtml = '<div class="chat-rolls-display-block"><div class="chat-rolls-header"><i class="fas fa-dice"></i>  </div><div class="chat-rolls-content">';
    ...
    let contextInjection = `\n\n[SYSTEM CONTEXT:   (Momentum): ${player.stats.momentum || 0} ( -5  5).  : ${activeTags.length > 0 ? activeTags.join(', ') : ''}.]`;
    ...
    if (text && typeof World !== 'undefined' && World) {
        ...
        if (matchedQueryIds.length > 0) {
            ...
            if (graphContext && graphContext.length > 0) {
                let recalledMemoryStr = `\n\n[RECALLED_MEMORY:    AriGraph  (${matchedQueryIds.join(', ')})]\n`;
                recalledMemoryStr += graphContext.map(n => {
                    let daysOld = Math.max(0, (World.current_day || 0) - (n.day || 0));
                    return `[${daysOld}d ago, ${n.location}] ${parseLocString(n.text)}`;
                }).join("\n");
                finalMessageForGM += recalledMemoryStr;
                addCalculationMessage(`[AriGraph]  ${graphContext.length}  : ${matchedQueryIds.join(', ')}`);
            }
        }
    }
    ...
    if (player.currentCombat && player.currentCombat.isActive) {
        ...
        if (activeEnemies.length === 0) {
            ...
            let enemiesData = activeEnemies; //    ID
            ...
            if (combatRes.success && combatRes.combat_log) {
                ...
                if (combatRes.total_damage > 0) {
                    ...
                    enemyRollsText += `   : ${combatRes.total_damage}. HP  .\n`;
                } else {
                    enemyRollsText += "  /  .\n";
                }
            }
            enemyRollsText += " :   (HP > 0)!         !   ,     !    logic_summary: ' .    '.]";
    ...
    player.lastTurnPulses = player.gameTime.totalPulses; //     
    ...
    if (turn > 0 && turn % MEMORY_PRUNE_TURN === 0) {
        addLogMessage(t('optimization.clearing', "     .      GM."), "command-feedback");
        conversationHistory = [];
    }
    ...
    if (currentHP > 0 && currentHP <= 15) {
        ...
        if (Math.random() * 100 < finalChance) {
            traumaInstruction = `\n\n[SYSTEM CRITICAL:    .       addStatusEffect.]`;
        }
    }

        let hasGuards = Object.values(player.visibleEntities).some(e => e.type === 'npc' && (e.profession?.toLowerCase().includes('') || (e.traits && e.traits.includes(''))));
    if (hasGuards) {
...
async function _internalPerformAiFetch(systemInstruction, history, providerModel, currentInput = "") {
        ...
        if (currentInput === "[INITIAL_GAME_SETUP_START_OF_STORY]") {
            return JSON.stringify({
                ...
                "actions": [
                    ...
                    { "command": "renderLocation", "args": { "locationId": "dummy_start", "size": "15x15", "description": " " } },
                    { "command": "addItem", "args": { "id": "sword_short_common", "name": " ", "slot": "right_hand", "effects": [{"type": "modify_stat", "stat": "str", "change": 1}] } },
                    { "command": "equipItem", "args": { "id": "sword_short_common", "slot": "right_hand" } },
                    { "command": "addItem", "args": { "id": "leather_armor_light_common", "name": " ", "slot": "torso", "effects": [{"type": "modify_stat", "stat": "res", "change": 2}] } },
                    { "command": "equipItem", "args": { "id": "leather_armor_light_common", "slot": "torso" } },
                    { "command": "addItem", "args": { "id": "boots_common", "name": " ", "slot": "feet", "effects": [{"type": "modify_stat", "stat": "res", "change": 1}] } },
                    { "command": "equipItem", "args": { "id": "boots_common", "slot": "feet" } },
                    { "command": "addItem", "args": { "id": "pants_common", "name": " ", "slot": "legs", "effects": [{"type": "modify_stat", "stat": "res", "change": 1}] } },
    ...
    if (timeSinceLastRequest < currentDelay) {
        ...
        console.log(`[Rate Limit]  ${waitTime}   ...`);
    ...
    // ----------------------------------------
        ...
        //   (Anthropic / Gemini  LLMost/OpenRouter)
        ...
        if (window.ModAPI && window.ModAPI.promptFilters && window.ModAPI.promptFilters.length > 0) {
            for (const filter of window.ModAPI.promptFilters) {
                ...
                } catch(e) { console.error("[ModAPI]    :", e); }
            }
        }
        // ------------------------------------------
        ...
        if (useThinkingMode) {
            finalTemperature = 1.0; //   Thinking   1.0
            finalMaxTokens = thinkingBudget + 4096; //    +    
            thinkingParams = {
                type: "enabled",
        ...
        if (currentApiProvider === 'local') {
            ...
            requestBody = {
                ...
                //  response_format: { type: "json_object" }    LM Studio
            };
            if (useThinkingMode) {
        ...
        console.log('   (' + currentApiProvider + ') : ' + targetUrl);
        ...
        try {
            ...
            if (response.status === 429 && currentApiProvider === 'gemini' && geminiApiKeys.length > 1) {
                console.warn('[KeyRotation]  #' + currentGeminiKeyIndex + '  (429).  ...');
            ...
            throw err;
                ...
                if (retry) {
                    ...
                    addLogMessage("[]      (Thinking/Caching),     ...", "system-message");
                    continue;
                }
                // FIX:   
...
async function handlePlayerDeath() {
    ...
    await executeCommand('addStatusEffect', {
        target: "player", id: "ghost_form", name: " ()", duration: 9999,
        description: " .   .   ,  .", effectsJSON: "[]"
    });

    await executeCommand('addQuest', {
        aiIdentifier: "quest_resurrection", title: "  ",
        objective: "        .",
        description: "  ,      .    .     ,  .",
        reward: "", issuer: ""
    });
        ...
        const deathPrompt = "[SYSTEM CRITICAL:     (HP   0).   ,          (Umbra)  , ,   .       .     (   ).  ,      (Aether Rift).      ,       .  :      ,  ,        .      setLocation, addQuest, addStatusEffect   HP   !    actions.]";
...
async function sendApiRequest(promptTextForAI, isInitialPrompt = false, isDiceRollResponse = false, expiredEffects = [], isSummarizationRequest = false, timeRetryCount = 0) {
    ...
    const thinkingTitle = isInitialPrompt ? " ..." : "  ...";
    const thinkingSub = isInitialPrompt ? "  " : "";
    ...
    try {
        ...
        if (isInitialPrompt) {
            ...
            for (const action of allPendingActions) {
                ...
                if (feedback) {
                    ...
                    if (typeof feedback === 'string' && feedback.includes("[ERROR]")) {
                        currentErrors.push(` ${action.command}   ${JSON.stringify(action.args)}  : ${feedback}`);
                    }
                }
            }
            ...
            if (!result || (!result.narrative && !result.actions)) throw new Error(" GM   .");
            if (!validateTime(result)) throw new Error("MISSING_TIME_PASSED");
            ...
            if (result.ai_reasoning) {
                addCalculationMessage(`[  (Reasoning)]:\n${result.ai_reasoning}`, "calc-info");
            }
            ...
            setTimeout(runAIPlayerTurn, 2500); //  2.5    -   
        }
...
if (!isInitialPrompt && !isSummarizationRequest && player.stats.turnCount > 0 && player.stats.turnCount % MEMORY_SUMMARY_TURN === 0) {
        ...
        if (error.message === "INCOMPLETE_RESPONSE" && timeRetryCount < 3) {
            console.warn(`GM   .  ${timeRetryCount + 1}  3...`);
            const timeErrorPrompt = promptTextForAI + "\n\n[ ]:     .       (narrative, actions  time_passed).    JSON.";
            const loaderSub = document.querySelector('.ether-text-subtitle');
            if (loaderSub) loaderSub.textContent = "  ...";
        ...
        if (error.message === "MISSING_TIME_PASSED" && timeRetryCount < 3) {
            console.warn(`GM   time_passed.  ${timeRetryCount + 1}  3...`);
            const timeErrorPrompt = promptTextForAI + "\n\n[ ]:     .       \"time_passed\"    JSON    . ,      (   5   ),   \"time_passed\": {\"days\": 0, \"hours\": 0, \"minutes\": 5}    .";
            const loaderSub = document.querySelector('.ether-text-subtitle');
            if (loaderSub) loaderSub.textContent = "  ...";
            return sendApiRequest(timeErrorPrompt, isInitialPrompt, isDiceRollResponse, expiredEffects, isSummarizationRequest, timeRetryCount + 1);
        }
        ...
        showAiErrorModal(
            ...
            isInitialPrompt,
            () => {
                if (isInitialPrompt) showLoadingScreen('loadingScreen.generatingWorld', ' ...');
...
async function runBackgroundSummarization() {
    ...
    try {
        ...
        // :   currentInput,  Gemini      contents
        const rawResponse = await performAiFetch(finalPrompt, [], modelId, "     (JSON).");
        const result = parseAIResponse(rawResponse);
        ...
        if (result.actions && result.actions.length > 0) {
            ...
            addLogMessage("    .", "command-feedback");
        } else {
            console.log("[Memory] GM        .");
        }
    } catch (e) {
        console.error("  :", e);
        addLogMessage("   .", "system-message");
...
function parseAIResponse(rawResponse) {
    // FIX:    undefined/null,  API      
    if (typeof rawResponse !== 'string') {
        console.warn("[Parser]     API,    .", rawResponse);
    ...
    if (startIdx !== -1 && endIdx !== -1) {
        ...
        try {
            ...
            for (let i = 0; i < fixedJsonString.length; i++) {
                ...
                if (char === '"' && !isEscaped) {
                    ...
                    if (inString) {
                        ...
                        else if (char.charCodeAt(0) < 32) sanitizedJson += ''; //    
            ...
            if (!parsed.time_passed) {
                console.warn("[Parser] GM  time_passed,  1 .");
...
function animateGoldChange(amount) {
    ...
    if (!isPositive) particle.style.color = '#e74c3c'; //   
...
/**
 *     (LOGIC)
 * : logic_rules + rules_and_instructions + combat_rules + env_guide + items_ref + snapshot
 */
...
function getPromptRuntimeConfig() {
  const defaults = {
    ...
    image_generation: {
      prompt_field_template: '"image_prompt": "!           .    .    .    : \'Ado music video aesthetic, monochrome anime style with one spot color, dark gothic, creepy vibe, extreme contrast, inverted colors, masterpiece, highly detailed\'.",',
      format_field_template: '"image_prompt": "        ().",'
    },
    response_languages: { ru: 'Russian', en: 'English', default: 'English' },
    unified_response: {
      default_time_passed: { days: 0, hours: 0, minutes: 5 },
      suggested_action_template: { text: '', roll_stat: null }
    },
    fallback_texts: {
...
function buildPromptSuggestedAction(runtimeConfig = getPromptRuntimeConfig()) {
  const action = runtimeConfig.unified_response.suggested_action_template || { text: '', roll_stat: null };
...
${style}
  ...
  ( ):  ${responseLanguage.toUpperCase()}!     "narrative", "director_notes"  "logic_summary"     .     !

###  ( ):
      ( ,  ,  )     .
      JSON.
: { ${buildImagePromptFormatField(promptRuntime)} "time_passed": ${buildPromptTimePassed(promptRuntime)}, "suggested_actions": [ ${buildPromptSuggestedAction(promptRuntime)} ], "narrative": "  ...", "actions": [ ... ... ], "logic_summary": "    ()" }
...
/**
 *     (NARRATIVE)
 * : narrative_rules + rules_and_instructions + skills_ref + lore + snapshot + logic_summary
 */
 ...
 *     GM       .
 *     ,       .
 * @param {string} text -    Gemini.
 * @returns {{narrative: string, commands: Array<object>}} -      .
 */
 ...
 *  ,   GM    .
 *  100%   ,   .
 * @param {string} command -     camelCase (, "addItem").
 * @param {object} args -      .
 * @returns {string|null} -       null,     .
 */
function applyTimePassed(tp) {
    ...
    if (totalPulses > 0) {
        ...
        let timeStrings = [];
        if (tp.days > 0) timeStrings.push(`${tp.days} .`);
        if (tp.hours > 0) timeStrings.push(`${tp.hours} .`);
        if (tp.minutes > 0) timeStrings.push(`${tp.minutes} .`);
        addCalculationMessage(`[] : ${timeStrings.join(', ')}.`);
    }
    return totalPulses;
}
...
function validateGMCommand(command, args) {
    ...
    switch (command) {
        case 'buildBusiness':
            if (!testArgs.facilityType || !FACILITY_NAMES[testArgs.facilityType]) return { valid: false, error: `   '${testArgs.facilityType}'. : ${Object.keys(FACILITY_NAMES).join(', ')}` };
            if (!testArgs.name || testArgs.name.trim() === '') return { valid: false, error: "     ." };
            break;
        case 'gmPurchaseGoods':
            if (!testArgs.factionId) return { valid: false, error: "  factionId." };
            if (!testArgs.regionId) return { valid: false, error: "  regionId." };
            if (!testArgs.goodType || !ECONOMY_ITEMS[testArgs.goodType]) return { valid: false, error: `   '${testArgs.goodType}'.` };
            if (!testArgs.quantity || isNaN(parseInt(testArgs.quantity)) || parseInt(testArgs.quantity) <= 0) return { valid: false, error: "    ." };
            if (typeof World !== 'undefined' && World) {
                let r = World.regions[testArgs.regionId];
                if (!r) return { valid: false, error: ` '${testArgs.regionId}'  .` };
                ...
                if (gold < cost) return { valid: false, error: `  '${testArgs.factionId}'  .  ${cost},  ${gold}.` };
                let stock = getRegionGoodStock(testArgs.regionId, testArgs.goodType);
                if (stock < testArgs.quantity) return { valid: false, error: `  '${testArgs.regionId}''${testArgs.goodType}'.  ${testArgs.quantity},  ${stock}.` };
            }
            break;
        case 'gmSellGoods':
            if (!testArgs.factionId) return { valid: false, error: "  factionId." };
            ...
            if (typeof World !== 'undefined' && World) {
                ...
                if (stock < testArgs.quantity) return { valid: false, error: `  '${testArgs.factionId}''${testArgs.goodType}'  .  ${testArgs.quantity},  ${stock}.` };
            }
            break;
        case 'gmInvestInFacility':
            if (!testArgs.factionId || !testArgs.regionId || !testArgs.facilityType || !testArgs.action) return { valid: false, error: "  ." };
            if (typeof World !== 'undefined' && World) {
            ...
            break;
        case 'gmSpreadRumor':
            if (!testArgs.factionId || !testArgs.targetFactionId || !testArgs.type || testArgs.investmentGold === undefined) return { valid: false, error: "  ." };
            if (typeof World !== 'undefined' && World) {
            ...
            break;
        case 'gmFrameForSabotage':
            if (!testArgs.factionId || !testArgs.targetFactionId || !testArgs.regionId) return { valid: false, error: "  ." };
            if (typeof World !== 'undefined' && World) {
                let gold = getFactionGold(testArgs.factionId);
                if (gold < 3000) return { valid: false, error: `  '${testArgs.factionId}'    .  3000,  ${gold}.` };
            }
            break;
        case 'gmDirectResourceInjection':
            if (!testArgs.regionId || !testArgs.goodType || !testArgs.quantity) return { valid: false, error: "  ." };
            if (typeof World !== 'undefined' && World) {
                let currentDay = Math.floor((World.tick || 0) / 24);
                let lastDay = World.lastDirectInjectionDay || -999;
                if (currentDay - lastDay < 7) {
                    return { valid: false, error: ` gmDirectResourceInjection  .  ${currentDay - lastDay}   7 .` };
                }
            }
            break;
        case 'gmDeclareWar':
            if (!testArgs.fromFactionId || !testArgs.toFactionId) return { valid: false, error: "  fromFactionId  toFactionId." };
            break;
        case 'gmForcePeace':
            if (!testArgs.factionId1 || !testArgs.factionId2) return { valid: false, error: "  factionId1  factionId2." };
            break;
        case 'gmChangeRulerTrait':
            if (!testArgs.rulerId || !testArgs.trait || testArgs.value === undefined) return { valid: false, error: "  rulerId, trait  value." };
            const allowedTraits = ['ambition', 'paranoia', 'wisdom', 'cruelty', 'diplomacy', 'military', 'stewardship'];
            if (!allowedTraits.includes(testArgs.trait)) return { valid: false, error: `  '${testArgs.trait}'. : ${allowedTraits.join(', ')}` };
            break;

        case 'startIntrigue':
            if (!testArgs.id || !testArgs.type || !testArgs.initiator || !testArgs.target) return { valid: false, error: "  id, type, initiator  target." };
            const allowedIntrigues = ['assassination', 'sabotage', 'rebellion', 'bribery'];
            if (!allowedIntrigues.includes(testArgs.type)) return { valid: false, error: `   '${testArgs.type}'. : ${allowedIntrigues.join(', ')}` };
            break;

        case 'startTravel':
            if (!testArgs.destinationId) return { valid: false, error: "  destinationId." };
            ...
            if (!destExists) return { valid: false, error: ` '${testArgs.destinationId}'    .    ID.` };
            break;
        case 'setPlayerDescription':
            if (!testArgs.text && !testArgs.description && !testArgs.bio && !testArgs.value && !testArgs.narrative && !testArgs.biography && !testArgs.background && !testArgs.history && !testArgs.lore) return { valid: false, error: "  text  description." };
            break;
        case 'updateStat':
        case 'setStat':
            if (!testArgs.stat) return { valid: false, error: "  stat." };
            ...
            if (!allowedStats.includes(baseStat) && baseStat !== 'reputation') {
                return { valid: false, error: `  '${testArgs.stat}' . : ${allowedStats.join(', ')}, reputation.*` };
            }
            break;
        case 'addEnvironment':
            if (!testArgs.id) return { valid: false, error: "  id." };
            if (!testArgs.name) return { valid: false, error: "   (name)." };
            if (!testArgs.type || !['npc', 'creature', 'enemy'].includes(testArgs.type)) return { valid: false, error: " (type)   npc, creature  enemy." };
            break;
        case 'renderLocation':
            // CityGen .    .
            break;
...
function validateActionsArray(actions) {
    ...
    for (let action of actions) {
        ...
        if (!val.valid) errors.push(` '${action.command}': ${val.error}`);
    }
    return errors;
}
...
async function executeNonInventoryCommand(command, args) {
    ...
    // ---   (  ) ---
    if (args && typeof args === 'object') {
        // 0.    entityKey  target  aiIdentifier
    ...
    updateDiceLogDisplay();
                        ...
                        feedback = `[ERROR] 'echoMemory'   .`;
                    }
                } else {
                    feedback = `[ERROR] 'echoMemory''text' (string).`;
                }
                break;
            case 'clearEchoMemory':
    ...
    updateDiceLogDisplay();
                    ...
                    feedback = `[ERROR] 'clearEchoMemory'   (confirm: true).`;
                }
                break;
            case 'removeEchoMemoryItem':
                if (player.echoMemory && player.echoMemory.items) {
                    if (args.index !== undefined && typeof args.index === 'number') {
                        ...
                        if (idx !== -1) {
                            ...
                            feedback = `[ERROR]  '${args.contains}'    -.`;
                        }
                    } else {
                        feedback = `[ERROR] 'removeEchoMemoryItem'  'index'  'contains'.`;
                    }
                    updateEchoMemoryDisplay();
    ...
    updateDiceLogDisplay();
            ...
            case 'setPlayerDescription':
                ...
                if (bioText) {
                    ...
                    feedback = `[ERROR] 'setPlayerDescription'  'text'  'description'. : ${JSON.stringify(args)}`;
                }
                break;
    ...
    if (window.ModAPI) ModAPI.emit('onLocationChanged', {newLocation: World.subLocations[locId]?.name, oldLocation: player.location});
                    ...
                    if (matchedLoc) {
                        ...
                        feedback = `[ ]  '${locName}'     !       (   ).      addSubLocation    addMapMarker.`;
                    }
                }
                ...
                if (foundLoc) {
                    ...
                    updateEnvironmentVisibility(); // -/ NPC
                    updateCharacterSheet();
                    updateMapDisplay();
                } else {
                    feedback = `[ERROR] 'setLocation''id'  'locationName'.`;
                }
                break;
        ...
        case 'startTravel':
                if (args.destinationId) {
                    ...
                    feedback = `[ERROR] 'startTravel'  'destinationId'.`;
                }
                break;
            case 'pauseTravel':
        ...
        case 'setJourneyLocation':
            if (player.currentJourney) {
                ...
                if (locId) {
                    ...
                    feedback = `[ERROR] 'setJourneyLocation'  'locationId'  'locationName'.`;
                }
            } else {
                feedback = `[ERROR] 'setJourneyLocation'       .`;
            }
            break;
        ...
        case 'calculationLog':
            ...
            case 'defineFaction':
                if (args.key && args.name) {
                    ...
                    feedback = `[DEBUG]  '${args.key}''${args.name}'.`;
                    addCalculationMessage(feedback);
                } else {
                    feedback = `[ERROR] 'defineFaction'  'key'  'name'.`;
                }
                break;
            ...
            case 'updateStat':
                ...
                if (args.stat && !isNaN(changeVal)) {
                    ...
                    if (stat === 'gold') {
                        if (change > 0) {
                            const addRes = await executeCommand('addItem', { aiIdentifier: 'gold', name: '', quantity: change });
                            if (addRes && addRes.includes("[")) {
                                feedback = addRes; //   
                                break;
                            }
                        } else if (change < 0) {
                    ...
                    if (stat === 'hp' && change > 0) {
                        ...
                        addCalculationMessage("[ ]    updateStat .  -!");
                        change = 0;
                    }
                    ...
                    if (stat === 'hp') {
                        ...
                        if (player.stats.hp <= player.stats.maxHp * 0.2) {
                            executeCommand('echoMemory', { text: `  ! (${player.stats.hp}/${player.stats.maxHp})` });
                        }
                        ...
                        if (Math.abs(change) >= 20) {
                            executeCommand('echoMemory', { text: `    ${change > 0 ? '+'+change : change} (: ${currentStatObject[finalStatName]})` });
                        }
                    } else if (stat === 'xp') {
                    ...
                    feedback = `[ERROR] 'updateStat'  'stat' (string)  'change' (number).`;
                }
                break;
            ...
            case 'setStat':
                if (args.stat && typeof args.value === 'number') {
                    ...
                    if (stat === 'gold') {
                        ...
                        if (diff > 0) {
                            const addRes = await executeCommand('addItem', { aiIdentifier: 'gold', name: '', quantity: diff });
                            if (addRes && addRes.includes("[")) {
                                feedback = addRes;
                                break;
                            }
                    ...
                    feedback = `[ERROR] 'setStat'  'stat' (string)  'value' (number).`;
                }
                break;
            ...
            case 'lockContainer':
                if (args.containerId) {
                    ...
                    if (cont) {
                        ...
                        feedback = `[]   ${args.containerId} .`;
                    } else {
                        feedback = `[ERROR]   .`;
                    }
                } else {
                    feedback = `[ERROR] 'lockContainer'  'containerId'.`;
                }
                break;
            ...
            case 'addQuest':
                if (args.aiIdentifier && args.title) {
                    ...
                    if (!existingQuest) {
                        ...
                        updateQuestList();
                        executeCommand('echoMemory', { text: `[] ${args.title}: ${args.objective}` });
                    } else {
                        feedback = t('gameInterface.commandFeedback.questAlreadyActive', { description: existingQuest.title });
                    }
                } else {
                    feedback = `[ERROR] 'addQuest'  'aiIdentifier'  'title'.`;
                }
                break;
                        ...
                        if (['active', 'completed', 'failed'].includes(newStatus)) {
                            if (quest.status !== newStatus) {
                                ...
                                if (newStatus === 'completed') {
                                    ...
                                    levelUp();
                                    feedback = t('gameInterface.commandFeedback.questStatusUpdated', { description: quest.title, status: statusLocalized }) + `  ${xpReward} XP.`;
                                    generateWorldNews(` ${player.name}   : "${quest.title}".`, player.location || "global", 2, 'misc');
                                } else {
                            ...
                            feedback = `[ERROR]    'updateQuest': ${args.status}.`;
                        }
                    } else {
                        feedback = t('gameInterface.commandFeedback.questNotFound', { questId: uqId });
                    }
                } else {
                    feedback = `[ERROR] 'updateQuest'  'aiIdentifier' ( 'title')  'status'.`;
                }
                break;
            ...
            case 'removeQuest':
                ...
                if (rqId) {
                    ...
                    feedback = `[ERROR] 'removeQuest'  'aiIdentifier' ( 'title').`;
                }
                break;
                ...
                if (eqId && args.field && args.value !== undefined) {
                    ...
                    if (questKey) {
                        ...
                        if (['title', 'objective', 'description', 'reward', 'issuer'].includes(field)) {
                            ...
                            feedback = `[ERROR]    'editQuest': ${args.field}.`;
                        }
                    } else {
                        feedback = t('gameInterface.commandFeedback.questNotFound', { questId: eqId });
                    }
                } else {
                    feedback = `[ERROR] 'editQuest'  'aiIdentifier' ( 'title'), 'field',  'value'.`;
                }
                break;
            ...
            case 'addSkill':
                if (args.id && args.name) {
                    ...
                    feedback = `[ERROR] 'addSkill'  'id'  'name'.`;
                }
                break;
            ...
            case 'addMapMarker':
                ...
                if (args.id && args.name && !isNaN(safeX) && !isNaN(safeY)) {
                    ...
                    if (attempts > 0) {
                        console.log(`[Map Collision]   .   '${args.name}'    (${args.x},${args.y})  (${Math.round(newX)},${Math.round(newY)}).`);
                    }
                    // --- [  ] ---
                    ...
                    feedback = `[ERROR] 'addMapMarker'  'id', 'name', 'x' (number),  'y' (number).`;
                }
                break;
            ...
            case 'removeMapMarker':
                if (args.id) {
                    ...
                    feedback = `[ERROR] 'removeMapMarker'  'id'.`;
                }
                break;
            ...
            case 'addSubLocation':
                if (args.id && args.name && args.parentId) {
                    ...
                    feedback = `[ ]   '${args.name}' (: ${args.parentId}).`;
                    updateMapDisplay();
                } else {
                    feedback = `[ERROR] 'addSubLocation'  'id', 'name'  'parentId'.`;
                }
                break;
            ...
            case 'buildBusiness':
                if (args.facilityType && args.name) {
                    ...
                    if (!playerRegionId) {
                        ...
                        feedback = `[]   .  '${player.location}'      .      -: ${availRegs}.  setLocation,     ,     .`;
                        break;
                    }
                            ...
                            try {
                                if (response.status === 'ok') {
                                    ...
                                    if (fullState && fullState.status === 'ok') {
                                        ...
                                        updateHoldingsDisplay();
                                        updateMapDisplay();
                                        addLogMessage(`[]  !   '${args.name}' .   14  .`, "command-feedback");
                                        generateWorldNews(` ${player.name}    '${args.name}'   ${World.regions[playerRegionId]?.name || playerRegionId}.`, playerRegionId, 3, 'business');
                                        showCustomAlert(` !      ''.       .`);
                                    } else {
                                        addLogMessage(`[]     .`, "system-message");
                                    }
                                } else {
                                    addLogMessage(`[ ] ${response.message || '    .'}`, "system-message");
                                }
                            } catch (err) {
                                addLogMessage(`[ ]  UI  : ${err.message}`, "system-message");
                                console.error("Business creation UI error:", err);
                            }
                        }).catch(err => {
                            addLogMessage(`[ ]  IPC  : ${err.message}`, "system-message");
                            console.error("Business creation IPC error:", err);
                        });
                        feedback = `[]    '${args.name}'  .    ...`;
                    }
                } else {
                    feedback = `[ERROR] 'buildBusiness'  'facilityType'  'name'.`;
                }
                break;
            ...
            case 'buyHolding':
                if (args.id && args.name && args.baseProfit) {
                    ...
                    feedback = `[]  : ${args.name}.  : ${args.baseProfit} /.`;
                    generateWorldNews(` ${player.name}  : ${args.name}.`, player.location || "global", 2, 'economy');
                    updateHoldingsDisplay();
                } else {
                    feedback = `[ERROR] 'buyHolding'  'id', 'name', 'baseProfit'.`;
                }
                break;
            ...
            case 'bankTransaction':
                ...
                if (args.type === 'deposit') {
                    if (player.stats.gold >= amount) {
                        ...
                        feedback = `[]  ${amount} .  : ${player.bankAccount.deposit} .`;
                    } else feedback = `[ERROR]    .`;
                    ...
                    feedback = `[]   ${amount} . : ${player.bankAccount.loanDays} .`;
                    ...
                    if (player.stats.gold >= amount) {
                        ...
                        feedback = `[]  ${actualRepay} .  : ${player.bankAccount.loan} .`;
                    } else feedback = `[ERROR]    .`;
                } else {
                    feedback = `[ERROR]   : ${args.type}`;
                }
                updateCharacterSheet();
                updateHoldingsDisplay();
                break;
            ...
            case 'nexusDefine':
                let dType = args.displayType || 'text'; //    
    ...
    updatePortPanel();
                    ...
                    feedback = `[ERROR] 'nexusDefine'  'id', 'name', 'category', 'value'.`;
                }
                break;
...
if (player.nexusData && player.nexusData[args.id]) {
    ...
    updatePortPanel();
                        ...
                        feedback = `[ERROR]  Nexus '${args.id}'  .`;
                    }
                } else {
                    feedback = `[ERROR] 'nexusUpdate'  'id'  'value'.`;
                }
                break;
    ...
    updatePortPanel();
            ...
            case 'repairFacility':
                if (args.regionId && args.facilityType) {
                    ...
                    if (fac && fac.durability < 100) {
                        fac.durability = 100;
                        feedback = `[] ${args.facilityType}  ${region.name} .`;
                    } else {
                        feedback = `[]    .`;
                    }
                }
            ...
            case 'applyPredefinedEffect':
                if (args.target === 'player' && args.effectId && typeof args.duration === 'number') {
                    const predefinedEffect = predefinedStatusEffects[args.effectId.toLowerCase()]; //      
                    ...
                    if (predefinedEffect) {
                        ...
                        //    ,     .
                        //        GM .
                        feedback = `[INFO] GM     '${args.effectId}'.        'addStatusEffect'   .`;
                        console.warn(feedback);
                    }
                } else {
                    feedback = `[ERROR] 'applyPredefinedEffect'  'target', 'effectId',  'duration'.`;
                }
                break;
            ...
            case 'addStatusEffect':
                if (args.target === 'player' && args.id && args.name && typeof args.duration === 'number' && args.description) {
                    ...
                    try {
                        ...
                        console.error(`  effectsJSON   '${args.id}':`, e, args.effectsJSON);
                    }
                    ...
                    feedback = `[ERROR] 'addStatusEffect'  'target', 'id', 'name', 'duration', 'description'.`;
                }
                break;
            ...
            case 'removeStatusEffect':
                if (args.target === 'player' && args.id) {
                    ...
                    feedback = `[ERROR] 'removeStatusEffect'  'target'  'id'.`;
                }
                break;

            case 'applyConsequence':
                if (!args.type) {
                    feedback = `[ERROR] 'applyConsequence'  'type' (pregnancy/disease/reputation).`;
                    break;
                }
            ...
            case 'updateRelationship':
                if (!args.npcId || !args.stat) {
                    feedback = `[ERROR] 'updateRelationship'  'npcId'  'stat' (affection/attraction/trust/intimacy).`;
                    break;
                }

                const npcForRel = player.allKnownEntities[args.npcId];
                if (!npcForRel) {
                    feedback = `[ERROR] NPC  ID '${args.npcId}'  .`;
                    break;
                }
                ...
                feedback = `  ${npcForRel.name}: ${stat} ${oldValue}  ${newValue} (${change >= 0 ? '+' : ''}${change})`;
                break;

            case 'recordIntimacy':
                if (!args.npcId) {
                    feedback = `[ERROR] 'recordIntimacy'  'npcId'.`;
                    break;
                }
                ...
                const intimacyRecord = {
                    ...
                    location: args.location || player.location || '',
                ...
                feedback = `    ${npcForIntimacy.name} (${args.type || 'consensual'}). Intimacy: +${intimacyIncrease}`;
                break;

            case 'recordEroticScene':
                const sceneText = args.narrative || window.lastGeneratedNarrative;
                if (!sceneText) {
                    feedback = `[ERROR] 'recordEroticScene'     .`;
                    break;
                }
                ...
                feedback = `     (${sceneRecord.partner},  ${sceneRecord.day}).`;
                break;
            ...
            case 'giveItem': {
                ...
                if (!rawId || !targetId) {
                    feedback = `[ERROR] 'giveItem'  'itemId'  'targetId'.`;
                    break;
                }
                ...
                if (!wNpc) {
                    if (player.allKnownEntities[targetId]) {
                        ...
                        feedback = `[ERROR] NPC  ID '${targetId}'  .`;
                        break;
                    }
                }
                ...
                if (res.success) {
                    ...
                    feedback = `[]   [${itemName} x${moveQty}]  ${wNpc.name}.`;
            ...
            case 'addEnvironment':
                if (args.aiIdentifier && args.name && args.type) {
                    ...
                    if (player.allKnownEntities[args.aiIdentifier]) {
                        ...
                        player.allKnownEntities[args.aiIdentifier] = {
                            ...
                            relationships: {
                                player: {
                                    ...
                                    trust: 0,          // 0-100 ()
                                    intimacy: 0,       // 0-100 (,   )
                                    sexualHistory: []  //  { day, location, type, timestamp }
                                }
                            }
                        };
                        ...
                        updateEnvironmentVisibility();
                        feedback = t('gameInterface.commandFeedback.entityAddedToEnv', { name: args.name }) + ` (: ${binding === 'player' ? '' : binding})`;
                    }
                } else {
                    feedback = `[ERROR] 'addEnvironment'  'aiIdentifier', 'name',  'type'.`;
                }
                break;
        ...
        const entityKey = args.aiIdentifier; //     
                    ...
                    if (entity) {
                        if (args.isDeath === true) {
                             ...
                             if (xpReward > 0) {
                                 player.stats.xp += xpReward;
                                 addCalculationMessage(`[XP]  ${xpReward}     ${entity.name}`);
                                 levelUp();
                             }
                        }
                        ...
                        if (args.isDeath === true && xp > 0) {
                            player.stats.xp += xp;
                            feedback = t('gameInterface.commandFeedback.entityRemovedFromEnv', { name: name }) + ` ( ${xp} )`;
                            levelUp();
                        } else {
                    ...
                    feedback = `[ERROR] 'removeEnvironment'  'aiIdentifier'.`;
                }
                break;
                ...
                if (args.aiIdentifier && args.stat && typeof args.value === 'number') {
                    ...
                    if (entId.startsWith("army_")) {
                        ...
                        if (foundArmy) {
                            ...
                            feedback = `[]  '${args.stat}'  ${entId}   ${args.value}.`;
                            addCalculationMessage(feedback);
                            break;
                        }
                    }
        ...
        const entityKey = args.aiIdentifier; //     
                        ...
                        if (entity.stats && ['hp', 'maxhp', 'str', 'dex', 'con', 'int'].includes(statName)) {
                            ...
                            if (systemStatName === 'hp') {
                                ...
                                if (validatedValue > currentHp) {
                                    ...
                                    addCalculationMessage(`[ ]   ${entity.name}  updateEntityStat .`);
                            ...
                            if (systemStatName === 'hp' && args.value <= 0) {
                                ...
                                generateWorldNews(` ${player.name}  : ${entity.name}.`, player.location || "global", 2, 'war');
                                //  
                            ...
                            feedback = `[ERROR]   '${args.stat}'  'updateEntityStat'.`;
                        }
                    } else {
                        feedback = t('gameInterface.commandFeedback.entityNotFoundInEnv', { id: args.aiIdentifier });
                    }
                } else {
                    feedback = `[ERROR] 'updateEntityStat'  'aiIdentifier', 'stat',  'value' (number).`;
                }
                break;
            ...
            case 'startIntrigue':
                if (args.id && args.type && args.initiator && args.target) {
                    ...
                    feedback = `[]   '${args.id}'  ${args.type}  ${args.target}.`;
                } else { feedback = `[ERROR] 'startIntrigue'  id, type, initiator, target.`; }
                break;
            case 'cancelIntrigue':
                if (args.id && World.intrigues) {
                    const idx = World.intrigues.findIndex(i => i.id === args.id);
                    if (idx !== -1) { World.intrigues.splice(idx, 1); feedback = `[]  '${args.id}' .`; }
                    else { feedback = `[ERROR]  '${args.id}'  .`; }
                }
                break;
            case 'revealIntrigue':
                if (args.id && World.intrigues) {
                    const intrigue = World.intrigues.find(i => i.id === args.id);
                    if (intrigue) { intrigue.isDiscovered = true; feedback = `[]  '${args.id}'  !`; generateWorldNews(`!    ${World.factions[intrigue.initiatorFactionId]?.name}  ${World.factions[intrigue.targetFactionId]?.name}!`, "global", 5, 'misc'); }
                }
                break;
            ...
            case 'overthrowRuler':
                if (args.factionId && World.factions[args.factionId]) {
                    ...
                    if (capitalRegionId && World.regions[capitalRegionId]?.vault_id) {
                        ...
                        generateWorldNews(`!   ${World.factions[args.factionId].name}  !  : ${weaponsLost} , ${foodLost} .`, "global", 5, 'war');
                    } else {
                        generateWorldNews(`!   ${World.factions[args.factionId].name}  !`, "global", 5, 'war');
                    }
                    generateWorldNews(`!   ${World.factions[args.factionId].name}  !`, "global", 5, 'war');
                    feedback = `[]     '${args.factionId}'.`;
                }
                break;
            case 'setFactionGoal':
                if (args.rulerId && World.rulers && World.rulers[args.rulerId]) {
                    World.rulers[args.rulerId].gmOverride = args.goal;
                    feedback = `[]   '${args.rulerId}'   : ${args.goal}.`;
                }
                break;
                ...
                if (typeof isActiveVal === 'boolean') {
                    ...
                    if (args.isActive) {
                        if (!wasActive || oldParticipants !== newParticipants) {
                            feedback = `[ ]  . : ${player.currentCombat.participants.join(', ')}`;
                    ...
                    feedback = `[ERROR] 'setCombatState'  'isActive' (boolean).`;
                }
                break;
...
case 'setEntityBinding':
                if (args.id && args.boundTo) {
                    ...
                    if (ent) {
                        ...
                        updateEnvironmentVisibility();
                        feedback = `[]  ${ent.name}  : ${args.boundTo}.`;
                    }
                }
                break;
        ...
        const entityKey = args.aiIdentifier; //     
                    ...
                    if (entity) {
                        if (args.property.toLowerCase() === 'ishostile') {
                            ...
                            feedback = `[DEBUG]    ${entity.name}   ${args.value}.`;
                            updateEnvironmentPanel();
                        } else {
                            feedback = `[ERROR]   '${args.property}'  'setEntityState'.`;
                        }
                    } else {
                        feedback = t('gameInterface.commandFeedback.entityNotFoundInEnv', { id: args.aiIdentifier });
                    }
                } else {
                    feedback = `[ERROR] 'setEntityState'  'aiIdentifier', 'property',  'value' (boolean).`;
                }
                break;
            ...
            case 'revealEntityTrait':
                if (args.id && args.trait) {
                    ...
                    if (entity) {
                        ...
                        if (traitAdded) {
                            feedback = `[]      ${entity.name}: ${args.trait}.`;
                            updateEnvironmentPanel();
                        } else {
                            feedback = `[DEBUG]  '${args.trait}'  ${entity.name}  .`;
                        }
                    } else {
                        feedback = `[ERROR]   ID '${args.id}'      revealEntityTrait.`;
                    }
                } else {
                    feedback = `[ERROR] 'revealEntityTrait'  'id'  'trait'.`;
                }
                break;
                ...
                if (args.aiIdentifier) {
                    ...
                    if (itemKey) {
                        ...
                        if (args._retries < 5) {
                            ...
                            feedback = `[ERROR]    '${args.aiIdentifier}'.     .`;
                        }
                    }
                } else {
                    feedback = `[ERROR] 'equipItem''aiIdentifier'.`;
            ...
            case 'updateItemStat':
                if (args.aiIdentifier && args.stat && args.change !== undefined) {
                    ...
                    if (item) {
                        ...
                        feedback = `[]  '${args.stat}'  '${item.custom_props.name}'   ${change > 0 ? '+' + change : change}.  : ${item.durability}`;
                        if (isEquipped) updateEquipmentDisplay();
                        else updateInventoryDisplay();
                    } else {
                        feedback = `[ERROR]  '${args.aiIdentifier}'    updateItemStat.`;
                    }
                } else {
                    feedback = `[ERROR] 'updateItemStat'  'aiIdentifier', 'stat'  'change'.`;
                }
                ...
                if (window.electronAPI && window.electronAPI.nexusGmIntervention) {
                    ...
                    if (res.status === 'ok') {
                        if (!res.feedback || res.feedback.trim() === '') {
                            let errMsg = `[  ]  '${command}'  C++ !    meterea_engine.exe   .`;
        ...
        feedback = t('gameInterface.commandFeedback.errorCommandGeneric', { command: command, args: error.message });
        console.error(`     ${command}:`, error, args);
    }
    return feedback;
}
...
function applyDisease(severity = 2) {
    ...
    severity = Math.max(1, Math.min(3, severity)); //  1-3
...
/**
 *    
 * @param {string} key -   (, 'sexual_reputation'  'scandal_npc_id')
 * @param {number} change -   ( )
 * @returns {string}   
 */
function applyReputationConsequence(key = 'sexual_reputation', change = -20) {
    ...
    updateCharacterSheet(); // :   ,   updateReputationDisplay  

    return ` : ${key} ${oldValue}  ${player.reputation[key]} (${change >= 0 ? '+' : ''}${change})`;
}

/**
 *    .
 * @param {string} itemInternalId -  ID   .
 * @returns {string|null}     null.
 */
async function equipItem(itemId, targetSlot = null) {
    ...
    if (existingItemInSlot) {
        ...
        if (unequipFeedback && unequipFeedback.includes(' ')) return unequipFeedback;
    }
...
async function handleDrop(event) {
    ...
    if (!isValid) {
        console.warn(`   '${draggedItemData.name}''${slotName}'.`);
        return;
    }
...
/**
 *     .
 * @param {string} slot -   (, 'head', 'right_hand').
 * @returns {string|null}     null.
 */
 ...
 *    ,   ,
 *      .
 * @returns {{effectiveStats: object, bonuses: object}}      .
 */
function getEffectiveStats() {
    ...
    if (totalWeight > carryLimit) {
        ...
        breakdown['dex'].push({ name: " ()", change: -penalty });
    }
...
function updateTraitsDisplay() {
    ...
    if (playerTraits.length === 0) {
        traitsList.innerHTML = `<li data-i18n="gameInterface.traitsPanel.empty">  </li>`;
        return;
    }
...
function updateHoldingsDisplay() {
    ...
    if (player.bankAccount.deposit > 0 || player.bankAccount.loan > 0) {
        ...
        bankHeader.innerHTML = '<i class="fas fa-university"></i>  ';
        holdingsList.appendChild(bankHeader);

        if (player.bankAccount.deposit > 0) {
            holdingsList.innerHTML += `<li title="    1%   "><span class="holding-name" style="color:#2ecc71"></span><span class="holding-value">${player.bankAccount.deposit} </span></li>`;
        }
        if (player.bankAccount.loan > 0) {
            holdingsList.innerHTML += `<li title="   2%  .  : ${player.bankAccount.loanDays}"><span class="holding-name" style="color:#e74c3c"> (${player.bankAccount.loanDays} .)</span><span class="holding-value">${player.bankAccount.loan} </span></li>`;
        }
    }
    ...
    propHeader.innerHTML = '<i class="fas fa-industry"></i> ';
    ...
    playerBusinesses.forEach(bus => {
        ...
        let statusText = bus.is_active ? `<span style="color:#f1c40f">${bus.cash_balance} </span>` : (bus.construction_days_left > 0 ? `<span style="color:#e67e22" title=" "> ${bus.construction_days_left} .</span>` : `<span style="color:#e74c3c"></span>`);
...
function handleBeforeUnload(event) {
    //           (  ).
    //  Electron  IPC- (invoke)    beforeunload 
    //   deadlock ( " "),      .
    console.log(" .  ...");
    stopAutoSaveTimer();
...
function changeBackground() {
    ...
    if (isVideo) {
        ...
        newElement.playsInline = true; //  iOS
    ...
    if (oldElement && oldElement !== elementToShow) { //          
        ...
        const removeOldElement = () => {
            if (oldElement && oldElement.parentNode === backgroundContainer) {
                ...
                // console.log("  :", oldElement.dataset.fileName);
            }
        };
        //       
        oldElement.addEventListener('transitionend', removeOldElement, { once: true });
        // Fallback,  transitionend   (,     display:none)
        setTimeout(removeOldElement, 2000); // 2 
    }
}
...
function startBackgroundChanger() {
    ...
    if (backgroundFiles.length > 0) {
        ...
        if (backgroundFiles.length > 1 && BACKGROUND_CHANGE_INTERVAL > 0) {
            ...
            console.log(`    ${backgroundFiles.length} .`);
        }
    } else {
        console.warn("    :  backgroundFiles .");
        if (backgroundContainer) backgroundContainer.style.backgroundColor = '#1a2530'; // Fallback 
    }
}
...
document.addEventListener('mousemove', (e) => {
    //   if (!player),   
    const moveX = (e.clientX - window.innerWidth / 2) * 0.01; //  1%
...
function updateSfxVolume() {
    hoverSfx.volume = sfxVolume * 0.2; // Hover  
...
document.addEventListener('mouseover', (e) => {
    ...
    if (target) {
        const now = Date.now();
        if (now - lastHoverSoundTime > 1000) { //  1  (debounce)
            playSfx(hoverSfx);
...
async function generateVisionImage(prompt) {
    ...
    console.log(`[Vision] . : ${imgProvider}, : ${imgModel || 'default'}`);
    ...
    if (imgProvider === 'llmost') {
        ...
        if (!key) throw new Error(' LLMost API Key    ');
    ...
    if (imgProvider !== 'pollinations') {
        ...
        if (!imageUrl) {
            console.error(' LLMost    :', data);
            throw new Error('   .    (nano-banana-2  gpt-5-image-mini)');
        }

        console.log('[Vision]    (base64)');
...
function populateAdminMenu() {
    ...
    let html = `
        <div style="margin-bottom: 20px; background: rgba(0,0,0,0.3); padding: 10px; border-radius: 5px; border-left: 3px solid #f1c40f;">
            <h4 style="margin: 0 0 10px 0; color: #f1c40f;">  </h4>
            <div style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
                ...
                <button data-action="admin-add-gold" style="background: #27ae60; margin: 0; padding: 5px 15px; min-width: auto;">+ </button>
                <button data-action="admin-heal" style="background: #e74c3c; margin: 0; padding: 5px 15px; min-width: auto;">Full HP/MP</button>
                <button data-action="admin-force-summary" style="background: #8e44ad; margin: 0; padding: 5px 15px; min-width: auto;">  (Summarize)</button>
                <button data-action="toggle-autotester" id="admin-autotester-btn" style="background: #e67e22; margin: 0; padding: 5px 15px; min-width: auto;"> -: </button>
                <button data-action="toggle-localization" style="background: #34495e; margin: 0; padding: 5px 15px; min-width: auto;"> : ${window.DISABLE_LOCALIZATION ? '' : ''}</button>
            </div>
        </div>

        <div style="margin-bottom: 20px; background: rgba(0,0,0,0.3); padding: 10px; border-radius: 5px; border-left: 3px solid #9b59b6;">
            <h4 style="margin: 0 0 10px 0; color: #9b59b6;">    </h4>
            <div style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
                <button onclick="adminSetTurn(28)" style="background: #8e44ad; margin: 0; padding: 5px 15px; min-width: auto;" title="     (29)"> = 28 ( )</button>
                <button onclick="adminSetTurn(29)" style="background: #8e44ad; margin: 0; padding: 5px 15px; min-width: auto;" title="     (30)"> = 29 ( )</button>
                <button onclick="adminInjectMemory()" style="background: #2980b9; margin: 0; padding: 5px 15px; min-width: auto;">  </button>
                <button onclick="runUnitTests()" style="background: #2c3e50; border: 1px solid #5dade2; margin: 0; padding: 5px 15px; min-width: auto;">  Unit-</button>
            </div>
            <p style="margin: 5px 0 0 0; font-size: 0.85em; color: #bdc3c7;"> : <b style="color:#fff">${player.stats.turnCount}</b></p>
        </div>
    `;
    ...
    html += `<div style="flex: 1; min-width: 300px; background: rgba(0,0,0,0.3); padding: 10px; border-radius: 5px; border-left: 3px solid #e74c3c;">
                <h4 style="margin: 0 0 10px 0; color: #e74c3c;">  (NPC/)</h4>
                <ul style="list-style: none; padding: 0; margin: 0;">`;

    const entities = Object.values(player.visibleEntities || {});
    if (entities.length === 0) html += `<li style="color: #7f8c8d; font-size: 0.9em;">  </li>`;
    entities.forEach(ent => {
        html += `<li style="margin-bottom: 8px; font-size: 0.9em; border-bottom: 1px dashed rgba(255,255,255,0.1); padding-bottom: 5px;">
                    ...
                    <div style="margin-top: 5px; display: flex; gap: 5px;">
                        <button onclick="adminKillEntity('${ent.id}')" style="background: #c0392b; padding: 2px 8px; font-size: 0.8em; margin: 0; min-width: auto;"> (0 HP)</button>
                        <button onclick="adminRemoveEntity('${ent.id}')" style="background: #7f8c8d; padding: 2px 8px; font-size: 0.8em; margin: 0; min-width: auto;"></button>
                    </div>
                 </li>`;
    });
    html += `</ul></div>`;
    ...
    html += `<div style="flex: 1; min-width: 300px; background: rgba(0,0,0,0.3); padding: 10px; border-radius: 5px; border-left: 3px solid #3498db;">
                <h4 style="margin: 0 0 10px 0; color: #3498db;">  (Nexus)</h4>
                <ul style="list-style: none; padding: 0; margin: 0;">`;

    const nexusItems = Object.values(player.nexusData || {});
    if (nexusItems.length === 0) html += `<li style="color: #7f8c8d; font-size: 0.9em;">  </li>`;
    nexusItems.forEach(item => {
        html += `<li style="margin-bottom: 8px; font-size: 0.9em; border-bottom: 1px dashed rgba(255,255,255,0.1); padding-bottom: 5px;">
                    <strong style="color: #5dade2">${item.name}</strong>: <span style="color: #f1c40f">${item.value}</span>
                    <div style="margin-top: 5px; display: flex; gap: 5px;">
                        <button onclick="adminEditNexus('${item.id}')" style="background: #2980b9; padding: 2px 8px; font-size: 0.8em; margin: 0; min-width: auto;"></button>
                        <button onclick="adminDeleteNexus('${item.id}')" style="background: #c0392b; padding: 2px 8px; font-size: 0.8em; margin: 0; min-width: auto;"></button>
                    </div>
                 </li>`;
    });
    html += `</ul></div>`;
    ...
    if (DEBUG_MODE && typeof World !== 'undefined' && World) {
        html += '<div style="margin-top: 20px; border-top: 2px solid #5dade2; padding-top: 10px;"><h3 style="color:#5dade2; margin-top:0;"> </h3></div>';
        
        // 1. 
        html += '<div class="debug-card"><div class="debug-card-title"><span> </span></div><div class="debug-grid">';
        for (let fId in World.factions) {
            let f = World.factions[fId];
            ...
            let warText = wars.length > 0 ? `<br><span style="color:#e74c3c; font-size:0.85em;"> : ${wars.join(', ')}</span>` : '';
            ...
            html += `<div class="debug-item" ${bgStyle}>
                     ...
                     ${formatNum(gold)} | ${formatNum(manpower)}
                     ${warText}
                     </div>`;
        }
        html += '</div></div>';
        ...
        // 2.   
        html += '<div class="debug-card"><div class="debug-card-title"><span>  </span></div>';
        ...
        for (let fId in World.factions) {
            World.factions[fId].armies.forEach(a => {
                ...
                let statusText = '';
                if (a.daysToMove > 0) {
                    statusText = `  ( ${a.daysToMove} .)`;
                } else if (a.siegeDays > 0) {
                    statusText = `<b style="color:#e67e22"></b> ( ${a.siegeDays} .)`;
                } else {
                    let phaseName = a.current_phase;
                    if (phaseName === 'vanguard_clash') phaseName = ' ';
                    else if (phaseName === 'main_battle') phaseName = ' ';
                    else if (phaseName === 'rout') phaseName = '';
                    statusText = `<b style="color:#e74c3c">: ${phaseName}</b>`;
                }

                let armyName = World.factions[fId].rulerId === 'player' ? "  " : World.factions[fId].name;

                html += `<div class="debug-army-item">
                            <b>${armyName}</b> (${a.size} .)  <b>${dest}</b><br>
                            : ${statusText}
                         </div>`;
            });
        }
        if (!armiesExist) html += '<div style="color:#7f8c8d; font-style:italic; padding:5px;">   </div>';
        html += '</div>';
        ...
        if(World.rulers) {
            for(let rId in World.rulers) {
                ...
                let goal = r.gmOverride ? `<b style="color:#e74c3c">[GM] ${r.gmOverride}</b>` : (r.currentGoal ? `${r.currentGoal.type} -> ${r.currentGoal.targetFactionId}` : " ");
                html += `<div class="debug-item">
                         <b style="color:#9b59b6">${r.name}</b> (${World.factions[r.factionId]?.name || r.factionId})<br>
                         HP: ${r.health}% | : ${r.personality.ambition}<br>
                         : ${goal}
                         </div>`;
            }
        }
        ...
        if(World.intrigues && World.intrigues.length > 0) {
            html += '<div style="margin-top:10px; border-top:1px dashed #555; padding-top:5px;"><b> :</b><br>';
            World.intrigues.forEach(i => {
                let phaseName = i.phase;
                if (phaseName === 'recruitment') phaseName = '';
                else if (phaseName === 'espionage') phaseName = '';
                else if (phaseName === 'execution') phaseName = '';
                else if (phaseName === 'cover_up') phaseName = ' ';
                html += `<span style="color:${i.isDiscovered ? '#e74c3c' : '#f39c12'}">[${i.type}]</span> ${i.initiatorFactionId} -> ${i.targetFactionId} (: ${phaseName}, : ${Math.floor(i.progress)}/${i.requiredProgress})<br>`;
            });
            html += '</div>';
        }
        html += '</div>';
    ...
    setTimeout(() => {
        ...
        if (btn) btn.innerHTML = isAutoTesting ? ' -: ' : ' -: ';
    }, 50);
...
window.adminAddGold = function () {
    ...
    executeCommand('addItem', { aiIdentifier: 'gold', name: '', quantity: val });
    populateAdminMenu();
};
...
window.adminForceSummary = async function () {
    closeAdminMenu();
    if (!player) return;
    addLogMessage("[ADMIN]    ...", "system-message");
    showLoadingScreen('loadingScreen.generatingWorld', ' ...');
    ...
    try {
        ...
        if (result.actions && result.actions.length > 0) {
            ...
            addLogMessage("GM     .", "command-feedback");
        }
    } catch (e) {
        console.error(e);
        addLogMessage(" : " + e.message, "system-message");
    } finally {
        hideLoadingScreen();
    }
};
...
window.adminEditNexus = function (id) {
    ...
    if (!item) return;
    const newVal = prompt(`    '${item.name}':`, item.value);
    if (newVal !== null) {
...
window.runUnitTests = function () {
    addLogMessage("[DEV]   Unit-...", "system-message");
    ...
    function assertEqual(testName, actual, expected) {
        if (actual === expected) {
            ...
            console.error(` [FAILED] ${testName} | : ${expected}, : ${actual}`);
            failed++;
        }
    }
    ...
    try {
        ...
        //  5:    (title -> id)
        let res5 = mockMiddleware('updateQuest', { title: ' ', status: 'completed' });
        assertEqual('Middleware: Quest title -> id', res5.id, ' ');
        assertEqual('Middleware: Quest title -> aiIdentifier', res5.aiIdentifier, ' ');
    ...
    const resultMsg = `[DEV]  . : ${passed}, : ${failed}.    (F12).`;
...
window.adminSetTurn = function (turnNumber) {
    ...
    addLogMessage(`[DEV]    ${turnNumber}.     ,    .`, "system-message");
    updateCharacterSheet();
    populateAdminMenu();
};
...
//    ;;;;
    ...
    player.gmNotes[`Test_NPC_${testId}`] = " .  .   ,      .";
    player.gmNotes[`Test_Lore_${testId}`] = " ,       .";

    addLogMessage(`[DEV]   GM  2   (NPC  Lore).`, "command-feedback");
    updateGmNotesDisplay();
    updateWorldSimDebugDisplay();
    populateAdminMenu();
};
...
async function runDeepSetupPipeline(narratorStyleGuide) {
    ...
    loaderDiv.innerHTML = `
        <div class="astrolabe">
            ...
            <div class="astrolabe-core"></div>
                    ...
                    <div class="ether-text-container">
                <span class="ether-text-title" id="deep-setup-title"> ...</span>
                <span class="ether-text-subtitle" id="deep-setup-sub"></span>
            </div>
            <button class="ether-cancel-btn" data-action="cancel-api">
                <i class="fas fa-times"></i>  
            </button>
    ...
    try {
        ...
        const getBaseContext = () => {
            const genderText = player.gender ? ` | : ${player.gender}` : '';
            return `: ${DEFAULT_WORLD_ID} | : ${player.era}\n: ${player.name} (${player.race}, ${player.class}${genderText})\n : ${player.startMode}\n  : "${player.description}"`;
        };
        ...
        let r1 = await performAiFetch(p1, [], modelIdForRequest, "    (JSON).");
        ...
        let r2 = await performAiFetch(p2, [], modelIdForRequest, "     (JSON).");
        ...
        let r3 = await performAiFetch(p3, [], modelIdForRequest, "    (JSON).");
        ...
        for (const action of (res3.actions || [])) {
            if (action.command === 'renderLocation' && enableLocalMap) {
                updateLoader(" 3/5:   ", "  ...");
                const locDesc = action.args.description || `: ${player.location}`;
        ...
        let r4 = await performAiFetch(p4, [], modelIdForRequest, "     (JSON).");
        ...
        if (typeof World !== 'undefined' && World) {
            ...
            if (World.news && World.news.length > 0) {
                ...
                if (epicHistory.length > 0) {
                    window.smartDeepContextStr += `\n===    ===\n` + epicHistory.map(n => `[ ${Math.floor((n.day||0)/360)+1} | ${n.location}]: ${parseLocString(n.text)}`).join("\n") + `\n       !\n`;
                }
            }
        }
        ...
        if (typeof World !== 'undefined' && World) {
            ...
            if (World.news && World.news.length > 0) {
                ...
                if (recentNews.length > 0) {
                    deepDynamicContext += `\n===   ( -) ===\n` + recentNews.map(n => `[${n.daysOld} . , : ${n.location}] ${parseLocString(n.text)}`).join("\n") + `\n      !\n`;
                }
            }
        }
        ...
        let r5 = await performAiFetch(p5, [], modelIdForRequest, "   (JSON).");
        ...
        if (!res5 || !res5.narrative) throw new Error("GM        5.");
        ...
        console.error(" Deep Setup:", error);
        removeLoader();
        showAiErrorModal(
            error.stack || error.message || String(error),
            true,
...
function updateWorldSimDebugDisplay() {
    ...
    if (DEBUG_MODE && typeof World !== 'undefined' && World) {
        ...
        html += `<div class="debug-card"><div class="debug-card-title"><span> ${t('extraLoc.debugPanel.factions')}</span></div><div class="debug-grid">`;
        ...
        for (let fId in World.factions) {
            ...
            let warText = wars.length > 0 ? `<br><span style="color:#e74c3c; font-size:0.85em;"> ${t('extraLoc.debugPanel.war')}: ${wars.join(', ')}</span>` : '';
                ...
                if (f.armies && f.armies.length > 0) {
                    ...
                    f.armies.forEach(a => {
                        ...
                        html += `<div style="background: rgba(0,0,0,0.4); padding: 6px; margin-top: 4px; border-radius: 4px; border-left: 2px solid #e74c3c;">
                            <span style="color:#ecf0f1;">${t('extraLoc.debugPanel.army')} (${a.size} .)</span><br>
        ...
        html += `<div class="debug-card"><div class="debug-card-title"><span> ${t('extraLoc.debugPanel.troopActivity')}</span></div>`;
        let armiesExist = false;
        for (let fId in World.factions) {
            World.factions[fId].armies.forEach(a => {
                armiesExist = true;
    ...
    for (let rId in World.rulers) {
        ...
        if (r.alive && (r.health <= 0 || r.stats.hp <= 0)) {
            ...
            if (r.heir && World.rulers[r.heir]) {
                ...
                generateWorldNews(` : ${r.name} .   ${heir.name}.`, "global", 5, 'misc');
                ...
                generateWorldNews(`: ${r.name} ,   !    .`, "global", 5, 'disaster');
                //   -  :    
...
//  processRulerDiplomacy  processIntrigues  -       world_worker.js
//     ,    main thread  
...
function checkRulerDeaths() {
    for (let rId in World.rulers) {
        ...
        if (r.alive && (r.health <= 0 || r.stats.hp <= 0)) {
            r.alive = false;
            if (World.npcs[rId]) World.npcs[rId].isAlive = false; //   NPC
            ...
            if (r.heir && World.rulers[r.heir]) {
                ...
                heir.profession = "";
                World.rulers[newRulerId] = heir;
                World.npcs[newRulerId] = heir; //   NPC
                World.factions[r.factionId].rulerId = newRulerId;
...
window.openBusinessModal = async function(bId) {
    ...
    if (bus.activity_logs && bus.activity_logs.length > 0) {
        logsHtml = bus.activity_logs.map(log => {
            ...
            let color = '#bdc3c7';
            if (parsedLog.includes('') || parsedLog.includes('')) color = '#2ecc71';
            if (parsedLog.includes('') || parsedLog.includes('-')) color = '#f39c12';
            if (parsedLog.includes('') || parsedLog.includes('') || parsedLog.includes('')) color = '#e74c3c';
            if (parsedLog.includes('')) color = '#3498db';
            ...
            <div class="bus-col-left">
                ...
                <div class="bus-card-modern">
                    ...
                    <div class="bus-flex-row">
                        <input type="number" id="bus-cash-input" placeholder="" class="bus-input">
                ...
                <div class="bus-card-modern">
                    ...
                    <div class="bus-stat-row" style="margin-bottom: 15px;">
                        ...
                        <span class="bus-stat-value">${bus.employee_count} / ${maxEmp} .</span>
                    </div>
                    <div class="bus-flex-row">
                ...
                <div class="bus-card-modern">
                    ...
                    <div style="display:flex; justify-content:space-between; align-items:center; margin: 15px 0 10px 0;">
                        ...
                        <span style="font-size: 0.85em; color: #f39c12;"><i class="fas fa-weight-hanging"></i> ${Math.round(currentWeight)} / ${maxWeight} </span>
                    </div>
...
`;
    ...
    bus.logistics.forEach(rule => {
        ...
        const amountText = rule.amount_is_percent ? `${rule.amount}%` : ` ${rule.amount} ${t('extraLoc.general.pcs')}`;
...
function updatePortPanel() {
    ...
    const yesStr = t('extraLoc.general.yes', null, '');
    const noStr = t('extraLoc.general.no', null, '');
    const existStr = t('extraLoc.general.exists', null, '');
    const notExistStr = t('extraLoc.general.notExists', null, '');
    ...
    if (port.dock_container_id && ContainerRegistry.has(port.dock_container_id)) {
        ...
        if (getContainerItems(dockCont).length === 0) {
            ...
            for (let proto in itemsMap) {
                html += `<li style="background: rgba(0,0,0,0.4); padding: 4px; margin-bottom: 2px; border-radius: 4px; border-left: 3px solid #f1c40f;"> ${getItemName(proto, player ? player.era : getRuntimeDefaultEraId())}: ${itemsMap[proto]} ${t('extraLoc.general.pcs', null, '.')}</li>`;
            }
        }
    } else {
    ...
    if (port.has_shipyard) {
        ...
        if (port.build_queue && port.build_queue.length > 0) {
            port.build_queue.forEach(bq => {
                html += `<li style="background: rgba(0,0,0,0.4); padding: 4px; margin-bottom: 2px; border-radius: 4px; border-left: 3px solid #e67e22;"> ${t('extraLoc.portPanel.shipTypes.' + bq.type, null, bq.type)} (${t('extraLoc.portPanel.owner')}: ${bq.owner_id}) - ${t('extraLoc.portPanel.daysLeft')}: ${bq.days_left}</li>`;
            });
        } else {
    ...
    shipsInPort.forEach(s => {
        let icon = "";
        if (s.type === "WAR_GALLEY" || s.type === "WAR_FRIGATE") icon = "";
        if (s.type === "PIRATE") icon = "";
...
window.addLogisticRule = async function(bId) {
    ...
    if (isPercent && amount > 100) return alert("     100.");
...
window.setBusinessEfficiency = async function(bId) {
    ...
    if (isNaN(eff) || eff < 0 || eff > 100) return alert("   0  100");
    if (window.electronAPI && window.electronAPI.nexusManageBusiness) {
...
window.setBusinessWages = async function(bId) {
    ...
    if (window.electronAPI && window.electronAPI.nexusManageBusiness) {
        ...
        if (res.status === 'ok') {
            ...
            addLogMessage(`[]       ${val}%.`, "command-feedback");
        }
    }
};
...
window.depositBusinessCash = async function(bId) {
    ...
    if (amount <= 0 || player.stats.gold < amount) return alert("   !");
    if (window.electronAPI && window.electronAPI.nexusManageBusiness) {
...
window.withdrawBusinessCash = async function(bId) {
    const amount = parseInt(document.getElementById('bus-cash-input').value) || 0;
    if (amount <= 0 || World.businesses[bId].cash_balance < amount) return alert("    !");
    if (window.electronAPI && window.electronAPI.nexusManageBusiness) {
...
window.toggleAutoTester = function() {
    ...
    if (isAutoTesting) {
        addLogMessage(`[DEV] - .  : ${aiPlayerTurnLimit > 0 ? aiPlayerTurnLimit : ''}.`, "system-message");
...
async function performAiPlayerFetch(systemInstruction, history, providerModel, currentInput) {
    ...
    if (isGeminiFormat) {
        if (data.candidates && data.candidates[0] && data.candidates[0].content && data.candidates[0].content.parts && data.candidates[0].content.parts[0]) {
            ...
            throw new Error("   Gemini: " + JSON.stringify(data));
        }
    } else {
        if (data.choices && data.choices[0] && data.choices[0].message) {
            resultText = data.choices[0].message.content;
        } else {
            throw new Error("   ( choices[0]): " + JSON.stringify(data));
        }
    }
...
async function runAIPlayerTurn() {
    ...
    if (aiPlayerTurnLimit > 0 && aiPlayerCurrentTurns >= aiPlayerTurnLimit) {
        ...
        addLogMessage(`[DEV] -   (   ${aiPlayerTurnLimit} ).`, "system-message");
        return;
    }

    aiPlayerCurrentTurns++;
    console.log(`[Auto-Tester]  ... ( ${aiPlayerCurrentTurns}/${aiPlayerTurnLimit > 0 ? aiPlayerTurnLimit : ''})`);
    
    const stateSnapshot = buildLitePlayerSnapshot();
    const recentLogs = player.gameLogHistory ? player.gameLogHistory.slice(-5).map(e => `${e.type === 'user-message' ? '' : 'GM'}: ${e.message}`).join('\n') : "  .";

    let systemPrompt = autoTesterPromptTemplate;
    if (!systemPrompt || systemPrompt.startsWith(":")) {
        systemPrompt = `    RPG. : ${stateSnapshot}\n: ${recentLogs}\n      (1-2 ).`;
    } else {
    ...
    try {
        const aiAction = await performAiPlayerFetch(systemPrompt, [], aiPlayerModelId, "   ?");
        
        if (!isAutoTesting) return; // ,      

        if (!aiAction || aiAction.trim() === "") {
            console.warn("[Auto-Tester]   . ,      (hit token limit).");
            addLogMessage(`[DEV] -   .  .`, "system-message");
            return;
        }
        ...
        if (userInput) {
            ...
            console.log("[Auto-Tester] :", userInput.value);
            handleUserInput();
        }
    } catch (e) {
        console.error("[Auto-Tester] :", e);
        addLogMessage(`[DEV]  -: ${e.message}.  .`, "system-message");
        isAutoTesting = false;
        const btn = document.getElementById('admin-autotester-btn');
        if (btn) btn.innerHTML = ' -: ';
    }
...
function restructureUI() {
    ...
    leftTabs.innerHTML = `
        <button class="s-tab-btn active" data-target="left-tab-hero"><i class="fas fa-user-shield"></i> </button>
        <button class="s-tab-btn" data-target="left-tab-gear"><i class="fas fa-shopping-bag"></i> </button>
    `;
    ...
    rightTabs.innerHTML = `
        <button class="s-tab-btn active" data-target="right-tab-env" title=""><i class="fas fa-map-marked-alt"></i></button>
        <button class="s-tab-btn" data-target="right-tab-journal" title=""><i class="fas fa-book-open"></i></button>
        <button class="s-tab-btn" data-target="right-tab-prog" title=""><i class="fas fa-dna"></i></button>
        <button class="s-tab-btn" data-target="right-tab-econ" title=""><i class="fas fa-coins"></i></button>
        <button class="s-tab-btn" data-target="right-tab-sys" title=""><i class="fas fa-cogs"></i></button>
    `;
...
async function openLoadWorldModal() {
    ...
    worldSlotsContainer.innerHTML = '<p style="text-align:center; color:#5dade2; padding: 20px;"><i class="fas fa-spinner fa-spin"></i>  ...</p>';
    ...
    if (worlds.length === 0) {
        worldSlotsContainer.innerHTML = '<p style="text-align:center; color:#7f8c8d; padding: 20px;">  .</p>';
        return;
    }
    ...
    worlds.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)).forEach(w => {
        ...
        btn.innerHTML = `
            <div style="display:flex; justify-content:space-between; width:100%;">
                ...
                <span style="color:#bdc3c7; font-size:0.85em;">: ${w.era}</span>
            </div>
            <div style="display:flex; justify-content:space-between; width:100%; margin-top:10px; align-items: center;">
                ...
                <div style="display:flex; gap: 5px;">
                    <button class="bus-btn btn-green load-w-btn" data-file="${w.filename}" style="padding:6px 12px; font-size:0.85em; margin:0; min-width:auto;"></button>
                    <button class="bus-btn btn-red del-w-btn" data-file="${w.filename}" style="padding:6px 12px; font-size:0.85em; margin:0; min-width:auto;"></button>
                </div>
            </div>
        `;
    ...
    worldSlotsContainer.querySelectorAll('.load-w-btn').forEach(b => {
        b.onclick = async (e) => {
            ...
            worldSlotsContainer.innerHTML = '<p style="text-align:center; color:#f1c40f; padding: 20px;"><i class="fas fa-spinner fa-spin"></i>  ...</p>';
            const wData = await window.electronAPI.loadWorldState(file);
            if (wData) {
                preloadedWorldData = wData;
                if (selectedWorldInfo) {
                    selectedWorldInfo.textContent = ` : ${wData.name || file}`;
...
function promptSaveWorldModal() {
    return new Promise((resolve) => {
        ...
        saveWorldNameInput.value = `_${player.era}_${new Date().toISOString().slice(0,10)}`;
